...
public:
	 ...
	 */
	explicit DateTime(int64_t ticks) : m_encoded(ticks)
	{
	}
	...
	{
		m_encoded = TimeSpan(static_cast<int64_t>(AbsoluteDays(year, doy) * TicksPerDay)).Ticks();
	}
	...
	static DateTime Now(bool useMicroseconds = false)
	{
		...
		if (useMicroseconds) {
			return DateTime(UnixEpoch +
			                duration_cast<microseconds>(system_clock::now().time_since_epoch()).count() * TicksPerMicrosecond);
		} else {
			return DateTime(UnixEpoch +
			                duration_cast<seconds>(system_clock::now().time_since_epoch()).count() * TicksPerSecond);
		}
	}
	...
	int AbsoluteDays(int year, int month, int day) const
	{
		 ...
		 */
		int result =
		    DayOfYear(year, month, day) - 1 + 365 * previousYear + previousYear / 4 - previousYear / 100 + previousYear / 400;
	...
	/**
	 * Convert to local mean sidereal time (GMST plus the observer's
	 * longitude)
