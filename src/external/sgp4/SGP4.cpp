...
void SGP4::Initialise()
{
	...
	RecomputeConstants(elements_.Inclination(), common_consts_.sinio, common_consts_.cosio, common_consts_.x3thm1,
	                   common_consts_.x1mth2, common_consts_.x7thm1, common_consts_.xlcof, common_consts_.aycof);
	...
	if (elements_.Period() >= 225.0) {
		...
		/*
		 * for perigee less than 220 kilometers, the simple_model flag
		 * is set and the equations are truncated to linear variation in
		 * sqrt a and quadratic variation in mean anomly. also, the c3
		 * term, the delta omega term and the delta m term are dropped
		 */
	...
	common_consts_.c4 =
	    2.0 * elements_.RecoveredMeanMotion() * coef1 * elements_.RecoveredSemiMajorAxis() * betao2 *
	    (common_consts_.eta * (2.0 + 0.5 * etasq) + elements_.Eccentricity() * (0.5 + 2.0 * etasq) -
	     2.0 * kCK2 * tsi / (elements_.RecoveredSemiMajorAxis() * psisq) *
	         (-3.0 * common_consts_.x3thm1 * (1.0 - 2.0 * eeta + etasq * (1.5 - 0.5 * eeta)) +
	          0.75 * common_consts_.x1mth2 * (2.0 * etasq - eeta * (1.0 + etasq)) * cos(2.0 * elements_.ArgumentPerigee())));
	...
	common_consts_.xnodot = xhdot1 + (0.5 * temp2 * (4.0 - 19.0 * theta2) + 2.0 * temp3 * (3.0 - 7.0 * theta2)) * common_consts_.cosio;
	...
	if (use_deep_space_) {
		...
		DeepSpaceInitialise(eosq, common_consts_.sinio, common_consts_.cosio, betao, theta2, betao2, common_consts_.xmdot,
		                    common_consts_.omgdot, common_consts_.xnodot);
	} else {
		double c3 = 0.0;
		if (elements_.Eccentricity() > 1.0e-4) {
			c3 = coef * tsi * kA3OVK2 * elements_.RecoveredMeanMotion() * kAE * common_consts_.sinio / elements_.Eccentricity();
		}
		...
		if (!use_simple_model_) {
			...
			nearspace_consts_.t5cof =
			    0.2 * (3.0 * nearspace_consts_.d4 + 12.0 * common_consts_.c1 * nearspace_consts_.d3 +
			           6.0 * nearspace_consts_.d2 * nearspace_consts_.d2 + 15.0 * c1sq * (2.0 * nearspace_consts_.d2 + c1sq));
		}
	}
}
...
Eci SGP4::FindPositionSDP4(double tsince) const
{
	...
	DeepSpaceSecular(tsince, elements_, common_consts_, deepspace_consts_, integrator_params_, xmdf, omgadf, xnode, em, xinc, xn);
	...
	DeepSpacePeriodics(tsince, deepspace_consts_, e, xinc, omgadf, xnode, xmam);
	...
	RecomputeConstants(xinc, perturbed_sinio, perturbed_cosio, perturbed_x3thm1, perturbed_x1mth2, perturbed_x7thm1, perturbed_xlcof,
	                   perturbed_aycof);

	/*
	 * using calculated values, find position and velocity
	 */
	return CalculateFinalPositionVelocity(elements_.Epoch().AddMinutes(tsince), e, a, omega, xl, xnode, xinc, perturbed_xlcof,
	                                      perturbed_aycof, perturbed_x3thm1, perturbed_x1mth2, perturbed_x7thm1, perturbed_cosio,
	                                      perturbed_sinio);
}

void SGP4::RecomputeConstants(const double xinc, double &sinio, double &cosio, double &x3thm1, double &x1mth2, double &x7thm1,
...
Eci SGP4::FindPositionSGP4(double tsince) const
{
	 ...
	 */
	return CalculateFinalPositionVelocity(elements_.Epoch().AddMinutes(tsince), e, a, omega, xl, xnode, xinc, common_consts_.xlcof,
	                                      common_consts_.aycof, common_consts_.x3thm1, common_consts_.x1mth2, common_consts_.x7thm1,
	                                      common_consts_.cosio, common_consts_.sinio);
}

Eci SGP4::CalculateFinalPositionVelocity(const DateTime &dt, const double e, const double a, const double omega, const double xl,
                                         const double xnode, const double xinc, const double xlcof, const double aycof, const double x3thm1,
                                         const double x1mth2, const double x7thm1, const double cosio, const double sinio)
{
	 ...
	 * - The fmod saves reduction of angle to +/-2pi in sin/cos() and
...
static inline double EvaluateCubicPolynomial(const double x, const double constant, const double linear, const double squared,
                                             const double cubed)
{
	return constant + x * linear + x * x * squared + x * x * x * cubed;
}

void SGP4::DeepSpaceInitialise(const double eosq, const double sinio, const double cosio, const double betao, const double theta2,
                               const double betao2, const double xmdot, const double omgdot, const double xnodot)
{
	...
	if (elements_.RecoveredMeanMotion() < 0.0052359877 && elements_.RecoveredMeanMotion() > 0.0034906585) {
		...
		deepspace_consts_.xlamo = Util::WrapTwoPI(elements_.MeanAnomoly() + elements_.AscendingNode() +
		                                          elements_.ArgumentPerigee() - deepspace_consts_.gsto);
		...
		deepspace_consts_.xlamo = Util::WrapTwoPI(elements_.MeanAnomoly() + elements_.AscendingNode() + elements_.AscendingNode() -
 ...
 */
void SGP4::DeepSpacePeriodics(const double tsince, const DeepSpaceConstants &ds_constants, double &em, double &xinc, double &omgasm,
...
void SGP4::DeepSpaceSecular(const double tsince, const OrbitalElements &elements, const CommonConstants &c_constants,
                            ...
                            double &xnodes, double &em, double &xinc, double &xn)
{
	...
	if (ds_constants.shape != DeepSpaceConstants::NONE) {
		...
		double xldot = 0.0;
		/*
		 * 1st condition (if tsince is less than one time step from
		 * epoch) 2nd condition (if atime and tsince are of opposite
		 * signs, so zero crossing required) 3rd condition (if tsince is
		 * closer to zero than atime, only integrate away from zero)
		 */
		if (fabs(tsince) < STEP || tsince * integ_params.atime <= 0.0 || fabs(tsince) < fabs(integ_params.atime)) {
		...
		bool running = true;
		while (running) {
			// always calculate dot terms ready for integration
			// beginning from the start of the range which is
			// 'atime'
			if (ds_constants.shape == DeepSpaceConstants::SYNCHRONOUS) {
				...
				xndot = ds_constants.d2201 * sin(x2omi + integ_params.xli - G22) +
				        ...
				        ds_constants.d4410 * sin(x2omi + x2li - G44) + ds_constants.d4422 * sin(x2li - G44) +
				        ds_constants.d5220 * sin(xomi + integ_params.xli - G52) +
				        ds_constants.d5232 * sin(-xomi + integ_params.xli - G52) +
				        ds_constants.d5421 * sin(xomi + x2li - G54) + ds_constants.d5433 * sin(-xomi + x2li - G54);
				xnddt = ds_constants.d2201 * cos(x2omi + integ_params.xli - G22) +
				        ...
				        2.0 * (ds_constants.d4410 * cos(x2omi + x2li - G44) + ds_constants.d4422 * cos(x2li - G44) +
				               ds_constants.d5421 * cos(xomi + x2li - G54) + ds_constants.d5433 * cos(-xomi + x2li - G54));
			}
			...
			if (fabs(ft) >= STEP) {
				...
				// integrate by a full step ('delt'), updating
				// the cached values for the new 'atime'
