...
public:
	 ...
	 */
	Tle(std::string line_one, std::string line_two) : line_one_(std::move(line_one)), line_two_(std::move(line_two))
	{
		Initialize();
	}

	/**
	 * @details Initialise given the satellite name and the two lines of a
	 * tle
	...
	Tle(std::string name, std::string line_one, std::string line_two)
	    : name_(std::move(name)), line_one_(std::move(line_one)), line_two_(std::move(line_two))
	{
		Initialize();
	}
