...
public:
	 ...
	 **********************************************************************/
	GeoCoords()
	    : _lat(Math::NaN()), _long(Math::NaN()), _easting(Math::NaN()), _northing(Math::NaN()), _gamma(Math::NaN()), _k(Math::NaN()),
	      _northp(false), _zone(UTMUPS::INVALID)
	{
		CopyToAlt();
	}
	 ...
	 *
	 * @param[in] s 1-element, 2-element, or 3-element string representation
	 *of the position.
	 ...
	 *
	 * Parse as a string and interpret it as a geographic position.  The
	 *input string is broken into space (or comma) separated pieces and
	 *Basic decision on which format is based on number of components
	 * -# MGRS
	 * -# "Lat Long" or "Long Lat"
	 * -# "Zone Easting Northing" or "Easting Northing Zone"
	 *
	 * The following inputs are approximately the same (Ar Ramadi Bridge,
	 *Iraq)
	 * - Latitude and Longitude
	 *   -  33.44      43.27
	 *   -  N33d26.4'  E43d16.2'
	 ...
	 * longitude precedes latitude in the absence of a hemisphere
	 *designator. Thus (with \e longfirst = false)
	 * - 40 -75
	 * - N40 W75
	 * - -75 N40
	 * - 75W 40N
	 ...
	 * minutes and seconds.  Various alternative symbols for degrees,
	 *minutes, and seconds are allowed.  Alternatively, use : to separate
	 *these components.  A single addition or subtraction is allowed.  (See
	 ...
	 * - 40:31-0:0.5
	 * .
	 * all specify the same angle.  The leading sign applies to the
	 *following components so -1d30 is -(1+30/60) = &minus;1.5.  However,
	 *note that -1:30-0:0:15 is parsed as (-1:30) + (-0:0:15) =
	 *&minus;(1+30/60) &minus; (15/3600).  Latitudes must be in the range
	 *[&minus;90&deg;, 90&deg;].  Internally longitudes are reduced to the
	 *range
	 * [&minus;180&deg;, 180&deg;].
	 *
	 * <b>UTM/UPS parsing</b>: For UTM zones (&minus;80&deg; &le; Lat <
	 * 84&deg;), the zone designator is made up of a zone number (for 1 to
	 *60) and a hemisphere letter (n or s), e.g., 38n (38north can also be
	 *used). The latitude band designer ([C--M] in the southern hemisphere
	 *and [N--X] in the northern) should NOT be used.  (This is part of the
	 *MGRS coordinate.)  The zone designator for the poles (where UPS is
	 *employed) is a hemisphere letter by itself, i.e., n or s (north or
	 *south can also be used).
	 *
	 * <b>MGRS parsing</b> interprets the grid references as square area at
	 *the specified precision (1m, 10m, 100m, etc.).  If \e centerp = true
	 *(the default), the center of this square is then taken to be the
	 *precise position; thus:
	 ...
	 **********************************************************************/
	explicit GeoCoords(const std::string &s, bool centerp = true, bool longfirst = false)
	{
	 ...
	 * @param[in] zone if specified, force the UTM/UPS representation to use
	 *a specified zone using the rules given in UTMUPS::zonespec.
	...
	{
		UTMUPS::Forward(latitude, longitude, _zone, _northp, _easting, _northing, _gamma, _k, zone);
		_lat = latitude;
	...
	{
		UTMUPS::Reverse(zone, northp, easting, northing, _lat, _long, _gamma, _k);
		_zone = zone;
		_northp = northp;
		_easting = easting;
		_northing = northing;
	 ...
	 * existing alternate representation.  Before this is called the
	...
	void SetAltZone(int zone = UTMUPS::STANDARD) const
	{
		...
		if (zone == _zone)
			...
			bool northp;
			UTMUPS::Forward(_lat, _long, _alt_zone, northp, _alt_easting, _alt_northing, _alt_gamma, _alt_k, zone);
		}
	}
	...
	/**
	 * String representation with latitude and longitude as degrees,
	 ...
	 * @return MGRS string.
	 *
	 * This gives the coordinates of the enclosing grid square with size
	 *given by the precision.  Thus 38n 444180 3684790 converted to a MGRS
	 ...
	 *notation for hemisphere; otherwise spell out the hemisphere
	 *(north/south)
	 * @exception GeographicErr if the hemisphere override attempts to
	 *change UPS N to UPS S or vice versa.
	 * @exception std::bad_alloc if memory for the string can't be
	 *allocated.
	 * @return UTM/UPS string representation: zone designator, easting, and
	 *   northing.
	...
	/**
	 * MGRS string for the alternate zone.  See
	 *GeoCoords::MGRSRepresentation.
	 *
	 * @param[in] prec precision (relative to about 1m).
	 * @exception std::bad_alloc if memory for the string can't be
	 *allocated.
	 ...
	 *(north/south)
	 * @exception GeographicErr if the hemisphere override attempts to
	 *change UPS n to UPS s or vice verse.
	 * @exception std::bad_alloc if memory for the string can't be
	 *allocated.
	 * @return UTM/UPS string representation: zone designator, easting, and
	 *   northing.
