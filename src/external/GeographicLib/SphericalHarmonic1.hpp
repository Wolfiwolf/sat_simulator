...
public:
	 ...
	 *\e C', and \e S'.  These arrays should not be altered or destroyed
	 *during the lifetime of a SphericalHarmonic object.
	 **********************************************************************/
	SphericalHarmonic1(const std::vector<real> &C, const std::vector<real> &S, int N, const std::vector<real> &C1,
	                   const std::vector<real> &S1, int N1, real a, unsigned norm = FULL)
	    : _a(a), _norm(norm)
	{
		if (!(N1 <= N))
	 ...
	 *during the lifetime of a SphericalHarmonic object.
	 **********************************************************************/
	SphericalHarmonic1(const std::vector<real> &C, const std::vector<real> &S, int N, int nmx, int mmx, const std::vector<real> &C1,
	                   const std::vector<real> &S1, int N1, int nmx1, int mmx1, real a, unsigned norm = FULL)
	    : _a(a), _norm(norm)
	{
		if (!(nmx1 <= nmx))
	 ...
	 *
	 * @param[in] tau multiplier for correction coefficients \e C' and \e
	 *S'.
	...
	Math::real operator()(real tau, real x, real y, real z) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<false, SphericalEngine::FULL, 2>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<false, SphericalEngine::SCHMIDT, 2>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		}
		return v;
	}
	...
	Math::real operator()(real tau, real x, real y, real z, real &gradx, real &grady, real &gradz) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<true, SphericalEngine::FULL, 2>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<true, SphericalEngine::SCHMIDT, 2>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		}
		return v;
	}
	 ...
	 *
	 * SphericalHarmonic1::operator()() exchanges the order of the sums in
	...
	CircularEngine Circle(real tau, real p, real z, bool gradp) const
	{
		...
		switch (_norm) {
		case FULL:
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::FULL, 2>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::FULL, 2>(_c, f, p, z, _a);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::SCHMIDT, 2>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::SCHMIDT, 2>(_c, f, p, z, _a);
			break;
		}
	}
