 ...
 **********************************************************************/
#define GEOGRAPHICLIB_RHUMBAREA_ORDER (GEOGRAPHICLIB_PRECISION == 2 ? 6 : (GEOGRAPHICLIB_PRECISION == 1 ? 4 : 8))
#endif
...
private:
	...
	// The following two functions (with lots of ignored arguments) mimic
	// the interface to the corresponding Geodesic function.  These are
	// needed by PolygonAreaT.
	void GenDirect(real lat1, real lon1, real azi12, bool, real s12, unsigned outmask, real &lat2, real &lon2, real &, real &, real &,
	               real &, real &, real &S12) const
	{
		GenDirect(lat1, lon1, azi12, s12, outmask, lat2, lon2, S12);
	}
	void GenInverse(real lat1, real lon1, real lat2, real lon2, unsigned outmask, real &s12, real &azi12, real &, real &, real &,
	                real &, real &S12) const
	{
...
public:
	...
	enum mask {
		...
		LONG_UNROLL = 1U << 15,
		/**
		 * Calculate everything.  (LONG_UNROLL is not included in this
		 *mask.)
		 * @hideinitializer
		 **********************************************************************/
		ALL = 0x7F80U,
	 ...
	 *of \e lon2 returned is in the range [&minus;180&deg;, 180&deg;].
	 *
	...
	{
		GenDirect(lat1, lon1, azi12, s12, LATITUDE | LONGITUDE | AREA, lat2, lon2, S12);
	}
	 ...
	 * meridians, there are two shortest rhumb lines and the east-going one
	 *is chosen.  \e lat1 and \e lat2 should be in the range
	 *[&minus;90&deg;, 90&deg;].  The value of \e azi12 returned is in the
	 *range
	...
	{
		GenInverse(lat1, lon1, lat2, lon2, DISTANCE | AZIMUTH | AREA, s12, azi12, S12);
	}
	...
	/**
	 * The general inverse rhumb problem.  Rhumb::Inverse is defined in
...
public:
	 ...
	 *from point 1.  The area is also computed.
	 *
	 * @param[in] s12 distance between point 1 and point 2 (meters); it can
	 *be negative.
	 * @param[out] lat2 latitude of point 2 (degrees).
	...
	/**
	 * Compute the position of point 2 which is a distance \e s12 (meters)
	 *from point 1.  The area is not computed.
	 **********************************************************************/
	...
	/**
	 * The general position routine.  RhumbLine::Position is defined in term
	 *so this function.
	 *
	 * @param[in] s12 distance between point 1 and point 2 (meters); it can
	 *be negative.
	 ...
	 *   wrapping it into the range [&minus;180&deg;, 180&deg;].
	 * .
	 * With the RhumbLine::LONG_UNROLL bit set, the quantity \e lon2 &minus;
	 *\e lon1 indicates how many times and in what sense the rhumb line
	 *encircles the ellipsoid.
	 *
	 * If \e s12 is large enough that the rhumb line crosses a pole, the
	...
	/**
	 * @return \e a the equatorial radius of the ellipsoid (meters).  This
	 *is the value inherited from the Rhumb object used in the constructor.
	 **********************************************************************/
	Math::real EquatorialRadius() const
	{
