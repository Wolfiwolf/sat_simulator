...
public:
	 ...
	 *
	 * The case of the letters in \e gars is ignored.  \e prec is in the
	 *range [0, 2] and gives the precision of \e gars as follows:
	 * - \e prec = 0 (min), 30' precision, e.g., 006AG;
	 * - \e prec = 1, 15' precision, e.g., 006AG3;
	 * - \e prec = 2 (max), 5' precision, e.g., 006AG39.
	 *
	 * If the first 3 characters of \e gars are "INV", then \e lat and \e
	 *lon are set to NaN and \e prec is unchanged.
	 **********************************************************************/
	...
	{
		return 1 / real(prec <= 0 ? mult1_ : (prec == 1 ? mult1_ * mult2_ : mult1_ * mult2_ * mult3_));
	}
