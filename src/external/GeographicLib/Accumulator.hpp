 ...
 **********************************************************************/
template <typename T = Math::real> class GEOGRAPHICLIB_EXPORT Accumulator
{
private:
	// _s + _t accumulators for the sum.
	T _s, _t;
	// Same as Math::sum, but requires abs(u) >= abs(v).  This isn't
	// currently used.
	...
	void Add(T y)
	{
		...
		y = Math::sum(y, _t, u);
		_s = Math::sum(y, _s, _t);
		// Start is _s, _t decreasing and non-adjacent.  Sum is now (s +
		// t + u) exactly with s, t, u non-adjacent and in decreasing
		// order (except for possible zeros).  The following code tries
		// to normalize the result. Ideally, we want _s = round(s+t+u)
		// and _u = round(s+t+u - _s).  The following does an
		// approximate job (and maintains the decreasing non-adjacent
		// property).  Here are two "failures" using 3-bit floats:
		//
		// Case 1: _s is not equal to round(s+t+u) -- off by 1 ulp
		// [12, -1] - 8 -> [4, 0, -1] -> [4, -1] = 3 should be [3, 0] =
		// 3
		//
		...
		//
		// "Fixing" these problems is probably not worth the expense.
		// The representation inevitably leads to small errors in the
		// accumulated values.  The additional errors illustrated here
		// amount to 1 ulp of the less significant word during each
		// addition to the Accumulator and an additional possible error
		// of 1 ulp in the reported sum.
		//
		// Incidentally, the "ideal" representation described above is
		// not canonical, because _s = round(_s + _t) may not be true.
		// For example, with 3-bit floats:
		//
...
public:
	/**
	 * Construct from a \e T.  This is not declared explicit, so that you
	 *can write <code>Accumulator<double> a = 5;</code>.
	 *
	 * @param[in] y set \e sum = \e y.
	 **********************************************************************/
	Accumulator(T y = T(0)) : _s(y), _t(0)
	{
		static_assert(!std::numeric_limits<T>::is_integer, "Accumulator type is not floating point");
	}
	/**
	 ...
	 *only integers such that \e n &times; \e T is exactly representable as
	 *a \e T (i.e., &plusmn; powers of two).  Use \e n = &minus;1 to negate
	 *\e sum.
	 *
