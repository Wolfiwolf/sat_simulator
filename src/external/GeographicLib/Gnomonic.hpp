...
private:
	...
	// precision (e.g., GEOGRAPHICLIB_DIGITS=2000) calculations.  Reverse
	// uses Newton's method which converges quadratically and so numit_ = 10
	// would normally be big enough.  However, since the Geodesic class is
	// based on a series it is of limited accuracy; in particular, the
	// derivative rules used by Reverse only hold approximately.
	// Consequently, after a few iterations, the convergence in the Reverse
	// falls back to improvements in each step by a constant (albeit small)
	// factor.
...
public:
	 ...
	 *calculations. By default this uses the WGS84 ellipsoid.
	 **********************************************************************/
	 ...
	 *lat, \e lon) (to within roundoff) provided the point in not over the
	 *horizon.
	 **********************************************************************/
	 ...
	 *the range [&minus;180&deg;, 180&deg;].  The scale of the projection is
	 * 1/<i>rk</i><sup>2</sup> in the "radial" direction, \e azi clockwise
	 *from true north, and is 1/\e rk in the direction perpendicular to
	 *this.  Even though all inputs should return a valid \e lat and \e lon,
	 *it's possible that the procedure fails to converge for very large \e x
	 *or \e y; in this case NaNs are returned for all the output arguments.
	 *A call to Reverse followed by a call to Forward will return the
	 *original (\e x, \e y) (to roundoff).
	 **********************************************************************/
