 ...
 *   <a
 *href="https://web.archive.org/web/20161214054445/http://earth-info.nga.mil/GandG/publications/tm8358.2/TM8358_2.pdf">
...
public:
	 ...
	 * The UTM divides the earth between latitudes &minus;80&deg; and
	 *84&deg; into 60 zones numbered 1 thru 60.  Zone assign zone number 0
	 *to the UPS regions, covering the two poles.  Within UTMUPS,
	 *non-negative zone numbers refer to one of the "physical" zones, 0 for
	 *UPS and [1, 60] for UTM.  Negative "pseudo-zone" numbers are used to
	 *select one of the physical zones.
	 **********************************************************************/
	enum zonespec {
		/**
		...
		MINPSEUDOZONE = -4,
		/**
		 * A marker for an undefined or invalid zone.  Equivalent to
		 *NaN.
		 **********************************************************************/
		INVALID = -4,
		/**
		 * If a coordinate already include zone information (e.g., it is
		 *an MGRS coordinate), use that, otherwise apply the
		 *UTMUPS::STANDARD rules.
		 **********************************************************************/
		MATCH = -3,
		/**
		 * Apply the standard rules for UTM zone assigment extending the
		 *UTM zone to each pole to give a zone number in [1, 60].  For
		 *example, use UTM zone 38 for longitude in [42&deg;, 48&deg;).
		 *The rules include the Norway and Svalbard exceptions.
		 **********************************************************************/
		UTM = -2,
		/**
		 * Apply the standard rules for zone assignment to give a zone
		 *number in [0, 60].  If the latitude is not in [&minus;80&deg;,
		 *84&deg;), then use UTMUPS::UPS = 0, otherwise apply the rules
		 *for UTMUPS::UTM.  The tests on latitudes and longitudes are
		 *all closed on the lower end open on the upper.  Thus for UTM
		 *zone 38, latitude is in [&minus;80&deg;, 84&deg;) and
		 *longitude is in [42&deg;, 48&deg;).
		 **********************************************************************/
		STANDARD = -1,
		/**
	 ...
	 *use that zone if it is non-negative, otherwise apply the rules given
	 *in UTMUPS::zonespec.
	 ...
	 *   90&deg;].
	 * @exception GeographicErr if the resulting \e x or \e y is out of
	 allowed
	 *   range (see Reverse); in this case, these arguments are unchanged.
	 *
	 * If \e setzone is omitted, use the standard rules for picking the
	 zone.
	 ...
	 * \code
	 ...
	 \endcode
	 **********************************************************************/
	static void Forward(real lat, real lon, int &zone, bool &northp, real &x, real &y, real &gamma, real &k, int setzone = STANDARD,
	 ...
	 *are allowed to be in in [0km, 9600km] for the northern hemisphere and
	 *in [900km, 10000km] for the southern hemisphere.  However UTM
	 *northings can be continued across the equator.  So the actual limits
	 *on the northings are [-9100km, 9600km] for the "northern" hemisphere
	 *and [900km, 19600km] for the "southern" hemisphere.
	 *
	 ...
	 * southern hemisphere.
	 *
	 * These ranges are 100km larger than allowed for the conversions to
	 *MGRS. (100km is the maximum extra padding consistent with eastings
	 *remaining non-negative.)  This allows generous overlaps between zones
	 *and UTM and UPS.  If \e mgrslimits = true, then all the ranges are
	 *shrunk by 100km so that they agree with the stricter MGRS ranges.  No
	 *checks are performed besides these (e.g., to limit the distance
	 ...
	 *
	 * @param[in] zonein the UTM zone for \e xin and \e yin (or zero for
	 *UPS).
	 * @param[in] northpin hemisphere for \e xin and \e yin (true means
	 ...
	 * @exception GeographicErr if \e xin or \e yin fall outside their
	 ...
	 *[0, 60] with \e zonein = UTMUPS::UPS, 0, indicating UPS.  \e zonein
	 *may also be UTMUPS::INVALID.
	 *
	 * \e zoneout must be in the range [UTMUPS::MINPSEUDOZONE,
	 *UTMUPS::MAXZONE] = [-4, 60].  If \e zoneout &lt; UTMUPS::MINZONE then
	 *the rules give in the documentation of UTMUPS::zonespec are applied,
	 *and \e zone is set to the actual zone used for output.
	 *
	 ...
	 * hemisphere letter, n or s (or "north" or "south" spelled out).  For
	 *UPS, it consists just of the hemisphere letter (or the spelled out
	 * hemisphere).  The returned value of \e zone is UTMUPS::UPS = 0 for
	 *UPS. Note well that "38s" indicates the southern hemisphere of zone 38
	 *and not latitude band S, 32&deg; &le; \e lat &lt; 40&deg;.  n, 01s,
	 *2n, 38s, south, 3north are legal.  0n, 001s, +3n, 61n, 38P are
	 *illegal.  INV is a special value for which the returned value of \e is
	 *UTMUPS::INVALID.
	 **********************************************************************/
	 ...
	 * @param[in] zone the UTM zone (zero means UPS).
	 * @param[in] northp hemisphere (true means north, false means south).
	 * @param[in] abbrev if true (the default) use abbreviated (n/s)
	 *notation for hemisphere; otherwise spell out the hemisphere
	 *(north/south)
	 ...
	 *in which case the returned string is "inv".  This reverses
	 * UTMUPS::DecodeZone.
	 **********************************************************************/
	 ...
	 *many different projections.  DecodeEPSG decodes those referring to UTM
	 *or UPS projections for the WGS84 ellipsoid.  If the code does not
	 *refer to one of these projections, \e zone is set to UTMUPS::INVALID.
	 *See https://www.spatialreference.org/ref/epsg/
	 **********************************************************************/
