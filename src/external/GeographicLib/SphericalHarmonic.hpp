...
public:
	...
	enum normalization {
		 ...
		 * <b>P</b><sub><i>n</i></sub><sup><i>m</i></sup>(\e z) is
		 *Ferrers function (also known as the Legendre function on the
		 *cut or the associated Legendre polynomial)
		 *https://dlmf.nist.gov/14.7.E10 and \e k = 1 for \e m = 0 and
		 *\e k = 2 otherwise.
		 *
...
public:
	 ...
	 * <i>S</i><sub><i>nm</i></sub> are stored in the one-dimensional
	 *vectors \e C and \e S which must contain (\e N + 1)(\e N + 2)/2 and \e
	 *N (\e N + 1)/2 elements, respectively, stored in "column-major" order.
	 *Thus for \e N = 3, the order would be: <i>C</i><sub>00</sub>,
	 * <i>C</i><sub>10</sub>,
	 * <i>C</i><sub>20</sub>,
	 * <i>C</i><sub>30</sub>,
	 * <i>C</i><sub>11</sub>,
	 ...
	 * (\e m &minus; 1)/2 + \e n.  The layout of \e S is the same except
	 *that the first column is omitted (since the \e m = 0 terms never
	 *contribute to the sum) and the 0th element is <i>S</i><sub>11</sub>
	 *
	 * The class stores <i>pointers</i> to the first elements of \e C and \e
	 *S. These arrays should not be altered or destroyed during the lifetime
	 *of a SphericalHarmonic object.
	 **********************************************************************/
	SphericalHarmonic(const std::vector<real> &C, const std::vector<real> &S, int N, real a, unsigned norm = FULL) : _a(a), _norm(norm)
	{
	...
	Math::real operator()(real x, real y, real z) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<false, SphericalEngine::FULL, 1>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<false, SphericalEngine::SCHMIDT, 1>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		}
		return v;
	}
	...
	Math::real operator()(real x, real y, real z, real &gradx, real &grady, real &gradz) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<true, SphericalEngine::FULL, 1>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<true, SphericalEngine::SCHMIDT, 1>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		}
		return v;
	}
	 ...
	 *
	 * SphericalHarmonic::operator()() exchanges the order of the sums in
	 the
	 ...
	 * SphericalHarmonic::Circle performs the inner sum over degree \e n
	 (which
	...
	CircularEngine Circle(real p, real z, bool gradp) const
	{
		...
		switch (_norm) {
		case FULL:
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::FULL, 1>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::FULL, 1>(_c, f, p, z, _a);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::SCHMIDT, 1>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::SCHMIDT, 1>(_c, f, p, z, _a);
			break;
		}
	}
