...
public:
	 ...
	 * Fourier series for the series conversions.  These are computed and
	 *saved when first needed.
	 **********************************************************************/
	 ...
	 *
	 * This allows a new AuxAngle to be initialized as an angle in radians
	 *with
	 * @code
	 ...
	 * @param[in] exact if true use the exact equations instead of the
	 *Taylor series [default false].
	 * @return the output auxiliary latitude \e eta as an AuxAngle.
	 *
	 * With \e exact = false, the Fourier coefficients for a specific \e
	 *auxin and \e auxout are computed and saved on the first call; the
	 *saved coefficients are used on subsequent calls.  The series method is
	 ...
	 *
	 * @param[in] exact if true use the exact expression instead of the
	 *Taylor series [default false].
	 ...
	 *
	 * @param[in] sinp if true sum the sine series, else sum the cosine
	 *series.
	 ...
	 *
	 * The result returned is \f$ \sum_0^{K-1} c_k \sin (2k+2)\zeta \f$, if
	 *\e sinp is true; with \e sinp false, replace sin by cos.
	 **********************************************************************/
...
protected:
	 ...
	 * @param[out] diff optional pointer to the derivative d tan(\e beta) /
	 *d tan(\e phi).
	 ...
	 *
	 * @param[in] phi geographic latitude.
	 * @param[out] diff optional pointer to the derivative d tan(\e theta) /
	 *d tan(\e phi).
	...
	{
		return (auxout >= 0 && auxout < AUXNUMBER && auxin >= 0 && auxin < AUXNUMBER) ? AUXNUMBER * auxout + auxin : -1;
	}
	// the function sqrt(1 + tphi^2), convert tan to sec
	static real sc(real tphi)
	{
		using std::hypot;
		return hypot(real(1), tphi);
	}
