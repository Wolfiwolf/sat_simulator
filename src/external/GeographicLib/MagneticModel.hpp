...
public:
	 ...
	 *the name.  If \e path is specified (and is non-empty), then the file
	 *is loaded from directory, \e path.  Otherwise the path is given by the
	 * DefaultMagneticPath().
	 *
	 ...
	 **********************************************************************/
	explicit MagneticModel(const std::string &name, const std::string &path = "", const Geocentric &earth = Geocentric::WGS84(),
	 ...
	 * @param[out] Bx the easterly component of the magnetic field
	 *(nanotesla).
	 ...
	 *
	 * @param[in] Bx the \e x (easterly) component of the magnetic field
	 *(nT).
	 * @param[in] By the \e y (northerly) component of the magnetic field
	 *(nT).
	 * @param[in] Bz the \e z (vertical, up positive) component of the
	 *magnetic field (nT).
	...
	{
		real Ht, Ft, Dt, It;
		FieldComponents(Bx, By, Bz, real(0), real(1), real(0), H, F, D, I, Ht, Ft, Dt, It);
	}

	/**
	 * Compute various quantities dependent on the magnetic field and its
	 *rate of change.
	 *
	 * @param[in] Bx the \e x (easterly) component of the magnetic field
	 *(nT).
	 ...
	 **********************************************************************/
	static void FieldComponents(real Bx, real By, real Bz, real Bxt, real Byt, real Bzt, real &H, real &F, real &D, real &I, real &Ht,
	                            real &Ft, real &Dt, real &It);
	///@}
	...
	/**
	 * @return date of the model, if available, from the ReleaseDate field
	 *in the data file; if absent, return "UNKNOWN".
	 **********************************************************************/
	...
	/**
	 * @return "name" used to load the magnetic model (from the first
	...
	/**
	 * @return the minimum time (in years) for which this MagneticModel
	 *should be used.
	 *
	 * Because the model will typically provide useful results
	...
	/**
	 * @return the maximum time (in years) for which this MagneticModel
	 *should be used.
	 *
	 * Because the model will typically provide useful results
	 * slightly outside the range of allowed times, no check of \e t
	 ...
	 *is the value of \e a inherited from the Geocentric object used in the
	 *   constructor.
	 **********************************************************************/
	 ...
	 * MagneticModel class does not use this function; it is just provided
	 *as a convenience for a calling program when constructing a
