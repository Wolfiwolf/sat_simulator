...
/// \cond SKIP
template <typename scalar_t> class kissfft;
/// \endcond
...
public:
	 ...
	 *
	 * The evaluates \f$ f(\sigma) \f$ at \f$ \sigma = (j + 1) \pi / (2 N)
	 *\f$ for integer \f$ j \in [0, N) \f$.  \e F should be an array of
	 *length at least \e N.
	 **********************************************************************/
	void GEOGRAPHICLIB_EXPORT transform(std::function<real(real)> f, real F[]) const;
	 ...
	 *   series; on output the refined transform based on 2\e N points,
	 *i.e., the first 2\e N coefficents.
	 *
	 * The evaluates \f$ f(\sigma) \f$ at additional points \f$ \sigma = (j
	 *+ \frac12) \pi / (2 N) \f$ for integer \f$ j \in [0, N) \f$, computes
	 *the DST-IV transform of these, and combines this with the input \e F
	 *to compute the 2\e N term DST-III discrete sine transform.  This is
	 * equivalent to calling transform with twice the value of \e N but is
	 *more efficient, given that the \e N term coefficients are already
	 *known.  See the example code above.
	 **********************************************************************/
	void GEOGRAPHICLIB_EXPORT refine(std::function<real(real)> f, real F[]) const;
	 ...
	 **********************************************************************/
	static real GEOGRAPHICLIB_EXPORT eval(real sinx, real cosx, const real F[], int N);
	 ...
	 * \f$ \sigma = \frac12\pi \f$.
	 **********************************************************************/
	static real GEOGRAPHICLIB_EXPORT integral(real sinx, real cosx, const real F[], int N);
	 ...
	 * &sigma;<sub>2</sub>.
	 **********************************************************************/
	static real GEOGRAPHICLIB_EXPORT integral(real sinx, real cosx, real siny, real cosy, const real F[], int N);
};
