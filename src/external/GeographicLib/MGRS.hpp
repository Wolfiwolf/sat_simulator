 ...
 *   <a
 *href="https://web.archive.org/web/20161214054445/http://earth-info.nga.mil/GandG/publications/tm8358.1/pdf/TM8358_1.pdf">
 ...
 * <a
 *href="https://earth-info.nga.mil/index.php?dir=wgs84&action=wgs84#tab_geotrans">geotrans</a>
...
private:
	...
	// Return latitude band number [-10, 10) for the given latitude
	...
	// Return approximate latitude band number [-10, 10) for the given
	// northing (meters).  With this rule, each 100km tile would have a
	// unique band letter corresponding to the latitude at the center of the
	// tile.  This function isn't currently used.
	static int ApproxLatitudeBand(real y)
	{
		...
		// convert to lat (mult by 90/100) and then to band (divide by
		// 8) the +1 fine tunes the boundary between bands 3 and 4
	...
	enum {
		...
		// Difference between S hemisphere northing and N hemisphere
		// northing
...
public:
	 ...
	 *northings are allowed to be in in [0 km, 9500 km] for the northern
	 *hemisphere and in [1000 km, 10000 km] for the southern hemisphere.
	 *(However UTM northings can be continued across the equator.  So the
	 *actual limits on the northings are [&minus;9000 km, 9500 km] for the
	 *"northern" hemisphere and [1000 km, 19500 km] for the "southern"
	 *hemisphere.)
	 *
	 * UPS eastings/northings are allowed to be in the range [1300 km, 2700
	 *km] in the northern hemisphere and in [800 km, 3200 km] in the
	 *southern hemisphere.
	 *
	 * The ranges are 100 km more restrictive than for the conversion
	 *between geographic coordinates and UTM and UPS given by UTMUPS.  These
	 * restrictions are dictated by the allowed letters in MGRS coordinates.
	 * The choice of 9500 km for the maximum northing for northern
	 *hemisphere and of 1000 km as the minimum northing for southern
	 *hemisphere provide at least 0.5 degree extension into standard UPS
	 *zones.  The upper ends of the ranges for the UPS coordinates is
	 *dictated by requiring symmetry about the meridians 0E and 90E.
	 *
	 * All allowed UTM and UPS coordinates may now be converted to legal
	 *MGRS coordinates with the proviso that eastings and northings on the
	 *upper boundaries are silently reduced by about 4 nm (4 nanometers) to
	 *place them \e within the allowed range.  (This includes reducing a
	 *southern hemisphere northing of 10000 km by 4 nm so that it is placed
	 *in latitude band M.)  The UTM or UPS coordinates are truncated to
	 *requested precision to determine the MGRS coordinate.  Thus in UTM
	 *zone 38n, the square area with easting in [444 km, 445 km) and
	 *northing in [3688 km, 3689 km) maps to MGRS coordinate 38SMB4488 (at
	 *\e prec = 2, 1 km), Khulani Sq., Baghdad.
	 *
	 * The UTM/UPS selection and the UTM zone is preserved in the conversion
	 *to MGRS coordinate.  Thus for \e zone > 0, the MGRS coordinate begins
	 *with the zone number followed by one of [C--M] for the southern
	 ...
	 *5nm of a band boundary.  For prec in [6, 11], the conversion is
	 *accurate to roundoff.
	 *
	 * If \e prec = &minus;1, then the "grid zone designation", e.g., 18T,
	 *is returned.  This consists of the UTM zone number (absent for UPS)
	 *and the first letter of the MGRS string which labels the latitude band
	 ...
	 * allocating a potentially large number of small strings.  If an error
	 *is thrown, then \e mgrs is unchanged.
	 **********************************************************************/
	static void Forward(int zone, bool northp, real x, real y, int prec, std::string &mgrs);

	/**
	 * Convert UTM or UPS coordinate to an MGRS coordinate when the latitude
	 *is known.
	 *
	 * @param[in] zone UTM zone (zero means UPS).
	 * @param[in] northp hemisphere (true means north, false means south).
	 ...
	 *   coordinates.
	 * @exception std::bad_alloc if the memory for \e mgrs can't be
	 *allocated.
	 *
	 * The latitude is ignored for \e zone = 0 (UPS); otherwise the latitude
	 *is used to determine the latitude band and this is checked for
	 *consistency using the same tests as Reverse.
	 **********************************************************************/
	 ...
	 * @param[in] centerp if true (default), return center of the MGRS
	 *square, else return SW (lower left) corner.
	 * @exception GeographicErr if \e mgrs is illegal.
	 *
	 * All conversions from MGRS to UTM/UPS are permitted provided the MGRS
	 * coordinate is a possible result of a conversion in the other
	 *direction. (The leading 0 may be dropped from an input MGRS coordinate
	 *for UTM zones 1--9.)  In addition, MGRS coordinates with a neighboring
	 ...
	 *   38[VW]LS); but 38VMS is not permitted (all of 38WMS is north of
	 *64N)
	 * - 38MPE and 38NPF are permitted (they straddle the equator); but
	 *38NPE and 38MPF are not permitted (the equator does not intersect
	 *either block).
	 ...
	 *With \e centerp = true, the conversion from MGRS to geographic and
	 *back is stable.  This is not assured if \e centerp = false.
	 *
	 * If a "grid zone designation" (for example, 18T or A) is given, then
	 *some suitable (but essentially arbitrary) point within that grid zone
	 *is returned.  The main utility of the conversion is to allow \e zone
	 *and \e northp to be determined.  In this case, the \e centerp
	 *parameter is ignored and \e prec is set to &minus;1.
	 *
	 * If the first 3 characters of \e mgrs are "INV", then \e x and \e y
	 *are set to NaN, \e zone is set to UTMUPS::INVALID, and \e prec is set
	 *to &minus;2.
	 *
	 ...
	 * @param[out] easting the leading digits of the block easting,
	 *e.g., 44.
	 * @param[out] northing the leading digits of the block easting,
	 *e.g., 88.
	 * @exception GeographicErr if \e mgrs is illegal.
	 *
	 * Only the most rudimentary checking of MGRS grid ref is done: it is
	 * expected to consist of 0-2 digits followed by 1 or 3 letters,
	 *followed (in the case of 3 letters) by an even number (possibly 0) of
	 *digits.  In reporting errors, the letters I and O (illegal in MSRS)
	 *are regarded as non-alphabetic.  The returned \e gridzone will always
	 *be non-empty.  The other output arguments may be empty strings.
	 *
	 * If the first 3 characters of \e mgrs are "INV", then \e gridzone is
	 *set to those 3 characters and the other return arguments are set to
	 *empty strings..
	 *
	 * If an exception is thrown, then the arguments are unchanged.
	 **********************************************************************/
	...
	/**
	 * Perform some checks on the UTMUPS coordinates on this ellipsoid.
	 *Throw an error if any of the assumptions made in the MGRS class is not
	 *true. This check needs to be carried out if the ellipsoid parameters
	 *(or the UTM/UPS scales) are ever changed.
	 **********************************************************************/
	static void Check();
};
