...
public:
	...
	/**
	 * @return \e A the total area of the ellipsoid (meters<sup>2</sup>).
	 *For a sphere \e A = 4&pi; <i>a</i><sup>2</sup>.  The radius of a
	 *sphere with the same area is sqrt(\e A / (4&pi;)).
	 **********************************************************************/
	Math::real Area() const;

	/**
	 * @return \e V the total volume of the ellipsoid (meters<sup>3</sup>).
	 *   For a sphere \e V = (4&pi; / 3) <i>a</i><sup>3</sup>.  The radius
	 *of a sphere with the same volume is cbrt(\e V / (4&pi;/3)).
	 **********************************************************************/
	 ...
	 *   ellipsoid.  This is the value used in the constructor.  This is
	 *zero, positive, or negative for a sphere, oblate ellipsoid, or prolate
	 *   ellipsoid.
	 **********************************************************************/
	 ...
	 *flattening of the ellipsoid.  This is zero, positive, or negative for
	...
	/**
	 * @return <i>e'</i> <sup>2</sup> = (<i>a</i><sup>2</sup> &minus;
	 *   <i>b</i><sup>2</sup>) / <i>b</i><sup>2</sup>, the second
	 *eccentricity squared of the ellipsoid.  This is zero, positive, or
	 ...
	 *<i>b</i><sup>2</sup>), the third eccentricity squared of the
	 ...
	 * - \e Z = \e b sin &beta;
	 * .
	 * where \e a and \e b are the equatorial radius and the polar
	 *semi-axis. For a sphere &beta; = &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.  The returned
	 *value &beta; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 * result is undefined if this condition does not hold.  The returned
	 *value &phi; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 * plane and a line between the center of the ellipsoid and a point on
	 *the ellipsoid.  For a sphere &theta; = &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.  The returned
	 *value &theta; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 *
	 * The rectifying latitude, &mu;, has the property that the distance
	 *along a meridian of the ellipsoid between two points with rectifying
	 *latitudes &mu;<sub>1</sub> and &mu;<sub>2</sub> is equal to
	 * (&mu;<sub>2</sub> - &mu;<sub>1</sub>) \e L / 90&deg;,
	 * where \e L = QuarterMeridian().  For a sphere &mu; = &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.  The returned
	 *value &mu; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.  The returned
	 *value &xi; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 *
	 * The conformal latitude, &chi;, gives the mapping of the ellipsoid to
	 *a sphere which which is conformal (angles are preserved) and in which
	 *the equator of the ellipsoid maps to the equator of the sphere.  For a
	 * sphere &chi; = &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.  The returned
	 *value &chi; lies in [&minus;90&deg;, 90&deg;].
	 **********************************************************************/
	 ...
	 * which which is conformal (angles are preserved) and in which the
	 *equator of the ellipsoid maps to a straight line of constant scale;
	 *this mapping defines the Mercator projection.  For a sphere &psi; =
	 * sinh<sup>&minus;1</sup> tan &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the result is
	 * undefined if this condition does not hold.  The value returned for
	 *&phi; = &plusmn;90&deg; is some (positive or negative) large but
	 *finite value, such that InverseIsometricLatitude returns the original
	 *value of &phi;.
	 **********************************************************************/
	 ...
	 *   between the equator and a point of latitude &phi; (meters).  \e s
	 *is given by \e s = &mu; \e L / 90&deg;, where \e L =
	 *   QuarterMeridian()).
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.
	 ...
	 *   and to the meridian.  &nu; is related to \e R = CircleRadius() by
	 *\e R = &nu; cos &phi;.
	 *
	 * &phi; must lie in the range [&minus;90&deg;, 90&deg;]; the
	 * result is undefined if this condition does not hold.
	 ...
	 *&minus; <i>b</i><sup>2</sup>) / <i>b</i><sup>2</sup>, the second
	 ...
	 *
	 * \e f should lie in (&minus;&infin;, 1).
	 * The returned value <i>e'</i> <sup>2</sup> lies in (&minus;1,
	 *&infin;).
	 **********************************************************************/
