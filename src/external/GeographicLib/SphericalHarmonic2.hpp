...
public:
	 ...
	 *\e N1 and \e N2 should satisfy \e N1 &le; \e N and \e N2 &le; \e N.
	 *
	 * The class stores <i>pointers</i> to the first elements of \e C, \e S,
	 *\e C', \e S', \e C'', and \e S''.  These arrays should not be altered
	 *or destroyed during the lifetime of a SphericalHarmonic object.
	 **********************************************************************/
	SphericalHarmonic2(const std::vector<real> &C, const std::vector<real> &S, int N, const std::vector<real> &C1,
	                   const std::vector<real> &S1, int N1, const std::vector<real> &C2, const std::vector<real> &S2, int N2, real a,
	                   unsigned norm = FULL)
	    : _a(a), _norm(norm)
	{
		if (!(N1 <= N && N2 <= N))
	 ...
	 *S.
	 * @param[in] nmx the maximum degree used in the sum.  The sum over \e n
	 *is from 0 thru \e nmx.
	 * @param[in] mmx the maximum order used in the sum.  The sum over \e m
	 *is from 0 thru min(\e n, \e mmx).
	 * @param[in] C1 the coefficients <i>C'</i><sub><i>nm</i></sub>.
	 * @param[in] S1 the coefficients <i>S'</i><sub><i>nm</i></sub>.
	 ...
	 *\e nmx &ge; \e mmx &ge; &minus;1; \e N1 &ge; \e nmx1 &ge; \e mmx1 &ge;
	 *   &minus;1; \e N &ge; \e N1; \e nmx &ge; \e nmx1; \e mmx &ge; \e
	 *mmx1; and similarly for \e N2, \e nmx2, and \e mmx2.
	 * @exception GeographicErr if any of the vectors of coefficients is not
	 *   large enough.
	 *
	 * The class stores <i>pointers</i> to the first elements of \e C, \e S,
	 ...
	 *or destroyed during the lifetime of a SphericalHarmonic object.
	 **********************************************************************/
	SphericalHarmonic2(const std::vector<real> &C, const std::vector<real> &S, int N, int nmx, int mmx, const std::vector<real> &C1,
	                   const std::vector<real> &S1, int N1, int nmx1, int mmx1, const std::vector<real> &C2,
	                   const std::vector<real> &S2, int N2, int nmx2, int mmx2, real a, unsigned norm = FULL)
	    : _a(a), _norm(norm)
	{
	 ...
	 * constructor for another object is initialized.  This default object
	 *can then be reset with the default copy assignment operator.
	 **********************************************************************/
	SphericalHarmonic2()
	{
	}
	 ...
	 *
	 * @param[in] tau1 multiplier for correction coefficients \e C' and \e
	 *S'.
	...
	Math::real operator()(real tau1, real tau2, real x, real y, real z) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<false, SphericalEngine::FULL, 3>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<false, SphericalEngine::SCHMIDT, 3>(_c, f, x, y, z, _a, dummy, dummy, dummy);
			break;
		}
		return v;
	}
	 ...
	 * @return \e V the spherical harmonic sum.
	 *
	 * This is the same as the previous function, except that the components
	 *of the gradients of the sum in the \e x, \e y, and \e z directions are
	 * computed.  This routine requires constant memory and thus never
	 *throws an exception.
	 **********************************************************************/
	Math::real operator()(real tau1, real tau2, real x, real y, real z, real &gradx, real &grady, real &gradz) const
	{
		...
		switch (_norm) {
		case FULL:
			v = SphericalEngine::Value<true, SphericalEngine::FULL, 3>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			v = SphericalEngine::Value<true, SphericalEngine::SCHMIDT, 3>(_c, f, x, y, z, _a, gradx, grady, gradz);
			break;
		}
		return v;
	}
	 ...
	 * points on a circle of latitude at fixed values of \e tau1 and \e
	 *tau2.
	 *
	 * @param[in] tau1 multiplier for correction coefficients \e C' and \e
	 *S'.
	 ...
	 *the definition, i.e., &sum;<sub><i>n</i> = 0..<i>N</i></sub>
	 ...
	 * CircularEngine::operator()() on the returned object performs the
	...
	CircularEngine Circle(real tau1, real tau2, real p, real z, bool gradp) const
	{
		...
		switch (_norm) {
		case FULL:
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::FULL, 3>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::FULL, 3>(_c, f, p, z, _a);
			break;
		case SCHMIDT:
		default: // To avoid compiler warnings
			return gradp ? SphericalEngine::Circle<true, SphericalEngine::SCHMIDT, 3>(_c, f, p, z, _a)
			             : SphericalEngine::Circle<false, SphericalEngine::SCHMIDT, 3>(_c, f, p, z, _a);
			break;
		}
	}
