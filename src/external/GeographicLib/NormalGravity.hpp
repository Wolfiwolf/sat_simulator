...
private:
	...
	static real atanzz(real x, bool alt)
	{
		...
		//
		// Require x >= -1.  Best to call with alt, s.t. x >= 0; this
		// results in a call to atan, instead of asin, or to asinh,
		// instead of atanh.
		using std::asin;
		using std::asinh;
		using std::atan;
		...
		using std::sqrt;
		real z = sqrt(fabs(x));
		return x == 0 ? 1 : (alt ? (!(x < 0) ? asinh(z) : asin(z)) / sqrt(fabs(x) / (1 + x)) : (!(x < 0) ? atan(z) : atanh(z)) / z);
	}
...
public:
	 ...
	 *   (meters<sup>3</sup>/seconds<sup>2</sup>); this is the product of \e
	 *G the gravitational constant and \e M the mass of the earth (usually
	 ...
	 * @param[out] geometricp if true (the default), then \e f_J2 denotes
	 *the flattening, else it denotes the dynamical form factor \e J2.
	 ...
	 * - geometrically (\e geomtricp = true), the ellipsoid is defined by
	 *the flattening \e f = (\e a &minus; \e b) / \e a, where \e a and \e b
	 *are the equatorial radius and the polar semi-axis.  The parameters
	 *should obey \e a &gt; 0, \e f &lt; 1.  There are no restrictions on \e
	 *GM or \e omega, in particular, \e GM need not be positive.
	 ...
	 * uninitialized object and is used by GravityModel which constructs
	 *this object before it has read in the parameters for the reference
	 *ellipsoid.
	 **********************************************************************/
	NormalGravity() : _a(-1)
	{
	 ...
	 *   (m s<sup>&minus;2</sup>).
	 *
	 * Due to the axial symmetry of the ellipsoid, the result is independent
	 *of the value of the longitude.  This acceleration is perpendicular to
	 *the surface of the ellipsoid.  It includes the effects of the earth's
	 * rotation.
	 **********************************************************************/
	 ...
	 *
	 * Due to the axial symmetry of the ellipsoid, the result is independent
	 *of the value of the longitude and the easterly component of the
	 * acceleration vanishes, \e gammax = 0.  The function includes the
	 *effects of the earth's rotation.  When \e h = 0, this function gives
	 *\e gammay = 0 and the returned value matches that of
	 ...
	 * &nabla;<i>V</i><sub>0</sub> + &nabla;&Phi; = <b>&Gamma;</b> +
	 *<b>f</b>.
	 **********************************************************************/
	 ...
	 *   (m<sup>2</sup> s<sup>&minus;2</sup>).
	 *
	 * This function excludes the centrifugal acceleration and is
	 *appropriate to use for space applications.  In terrestrial
	 *applications, the function NormalGravity::U (which includes this
	 *effect) should usually be used.
	 **********************************************************************/
	 ...
	 *   (m<sup>3</sup> s<sup>&minus;2</sup>).  This is the value used in
	 *the constructor.
	 **********************************************************************/
	 ...
	 * used in the constructor.  Otherwise it is the zonal coefficient of
	 *the Legendre harmonic sum of the normal gravitational potential.  Note
	 *that <i>J</i><sub><i>n</i></sub> = 0 if \e n is odd.  In most gravity
	...
	/**
	 * @return <i>f</i> the flattening of the ellipsoid (\e a &minus; \e
	 *b)/\e a.
	 **********************************************************************/
	 ...
	 *&minus; <i>omega</i><sup>2</sup><i>a</i><sup>3</sup>/<i>GM</i>
	 *8/(45&pi;).  A NaN is returned if these conditions do not hold.  The
	 *restriction to positive \e GM is made because for negative \e GM two
