...
public:
	/**
	 * Bit masks for the capabilities to be given to the GravityCircle
	 *object produced by Circle.
	 **********************************************************************/
	enum mask {
		/**
		 * No capabilities.
		...
		GRAVITY = CAP_G,
		/**
		 * Allow calls to GravityCircle::Disturbance and
		 *GravityCircle::T.
		 * @hideinitializer
		 **********************************************************************/
		DISTURBANCE = CAP_DELTA,
		/**
		 * Allow calls to GravityCircle::T(real lon) (i.e., computing
		 *the disturbing potential and not the gravity disturbance
		 *vector).
		 * @hideinitializer
		 **********************************************************************/
	 ...
	 * @param[in] Nmax (optional) if non-negative, truncate the degree of
	 *the model this value.
	 ...
	 * @exception std::bad_alloc if the memory necessary for storing the
	 *model can't be allocated.
	 *
	 ...
	 * After the model is loaded, the maximum degree and order of the model
	 *can be found by the Degree() and Order() methods.
	 **********************************************************************/
	explicit GravityModel(const std::string &name, const std::string &path = "", int Nmax = -1, int Mmax = -1);
	///@}
	...
	/**
	 * Evaluate the gravity disturbance vector at an arbitrary point above
	 ...
	 * results of the NGA codes are reproduced accurately.  Details are
	 *given in \ref gravitygeoid.
	 **********************************************************************/
	 ...
	 * @param[out] xi the northerly component of the deflection of the
	 *vertical (degrees).
	 * @param[out] eta the easterly component of the deflection of the
	 *vertical (degrees).
	 *
	 * The spherical approximation (see Heiskanen and Moritz, Sec 2-14) is
	 *used so that the results of the NGA codes are reproduced accurately.
	...
	/**
	 * Evaluate the components of the acceleration due to gravity in
	...
	///@{
	/**
	 * Create a GravityCircle object to allow the gravity field at many
	 *points with constant \e lat and \e h and varying \e lon to be computed
	 * efficiently.
	 *
	 ...
	 * .
	 * The default value of \e caps is GravityModel::ALL which turns on all
	 *the capabilities.  If an unsupported function is invoked, it will
	 *return NaNs.  Note that GravityModel::GEOID_HEIGHT will only be
	 *honored if \e h = 0.
	 *
	 ...
	 * models.  See \ref gravityparallel for an example of using
	 *GravityCircle (together with OpenMP) to speed up the computation of
	 *geoid heights.
	 **********************************************************************/
	...
	/**
	 * @return the description of the gravity model, if available, in the
	...
	/**
	 * @return "name" used to load the gravity model (from the first
	 *argument of the constructor, but this may be overridden by the model
	 *file).
	 **********************************************************************/
	 ...
	 *   s<sup>&minus;2</sup>); this is the product of \e G the
	 ...
	 * GravityModel class does not use this function; it is just provided as
	 *a convenience for a calling program when constructing a GravityModel
	 * object.
	 **********************************************************************/
