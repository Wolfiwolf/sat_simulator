...
public:
	...
	enum flag {
		...
		LONGITUDE = 2,
		/**
		 * Used in Encode to indicate output of an azimuth in [000, 360)
		 *with no letter indicator.
		 * @hideinitializer
		 **********************************************************************/
		AZIMUTH = 3,
		/**
...
public:
	 ...
	 * (single quote), &quot; (double quote), and these components may only
	 *be given in this order.  Any (but not all) components may be omitted
	 *and other symbols (e.g., the &deg; symbol for degrees and the unicode
	 *prime and double prime symbols for minutes and seconds) may be
	 *substituted; two single quotes can be used instead of &quot;.  The
	 *last component indicator may be omitted and is assumed to be the next
	 *smallest unit (thus 33d10 is interpreted as 33d10').  The final
	 *component may be a decimal fraction but the non-final components must
	 *be integers.  Instead of using d, ', and &quot; to indicate degrees,
	 *minutes, and seconds, : (colon) may be used to <i>separate</i> these
	 ...
	 *components must be less than 60.  A single leading sign is permitted.
	 *A hemisphere designator (N, E, W, S) may be added to the beginning or
	 *end of the string.  The result is multiplied by the implied sign of
	 *the hemisphere designator (negative for S and W).  In addition \e ind
	 *is set to DMS::LATITUDE if N or S is present, to DMS::LONGITUDE if E
	 *or W is present, and to DMS::NONE otherwise.  Leading and trailing
	 *whitespace is removed from the string before processing.  This routine
	 *throws an error on a malformed string.  No check is performed on the
	 *range of the result. Examples of legal and illegal strings are
	 ...
	 *   - 4d0'9, 4d9&quot;, 4d9'', 4:0:9, 004:00:09, 4.0025, 4.0025d,
	 *4d0.15, 04:.15
	 *   - 4:59.99999999999999, 4:60.0, 4:59:59.9999999999999, 4:59:60.0, 5
	 * - <i>ILLEGAL</i> (the exception thrown explains the problem)
	 *   - 4d5&quot;4', 4::5, 4:5:, :4:5, 4d4.5'4&quot;, -N20.5, 1.8e2d,
	 *4:60, 4:59:60
	 *
	 * The decoding operation can also perform addition and subtraction
	 * operations.  If the string includes <i>internal</i> signs (i.e., not
	 *at the beginning nor immediately after an initial hemisphere
	 *designator), then the string is split immediately before such signs
	 *and each piece is decoded according to the above rules and the results
	 *added; thus <code>S3-2.5+4.1N</code> is parsed as the sum of
	 *<code>S3</code>, <code>-2.5</code>, <code>+4.1N</code>.  Any piece can
	 *include a hemisphere designator; however, if multiple designators are
	 *given, they must compatible; e.g., you cannot mix N and E.  In
	 *addition, the designator can appear at the beginning or end of the
	 *first piece, but must be at the end of all subsequent pieces (a
	 *hemisphere designator is not allowed after the initial sign). Examples
	 *of legal and illegal combinations are
	 ...
	 * <code>8.0E</code>.
	 *
	 * \note At present, all the string handling in the C++ implementation
	 *of %GeographicLib is with 8-bit characters.  The support for unicode
	 * symbols for degrees, minutes, and seconds is therefore via the
	 * <a href="https://en.wikipedia.org/wiki/UTF-8">UTF-8</a> encoding.
	 *(The JavaScript implementation of this class uses unicode natively, of
	 * course.)
	 *
	 * Here is the list of Unicode symbols supported for degrees, minutes,
	 * seconds, and the plus and minus signs; various symbols denoting
	 *variants of a space, which may separate the components of a DMS
	 *string, are removed:
	 * - degrees:
	 ...
	 * .
	 * The codes with a leading zero byte, e.g., U+00b0, are accepted in
	 *their UTF-8 coded form 0xc2 0xb0 and as a single byte 0xb0.
	 **********************************************************************/
	 ...
	 * @param[in] longfirst if true assume longitude is given before
	 *latitude in the absence of hemisphere designators (default false).
	 ...
	 * @exception GeographicErr if \e dmsa and \e dmsb are both interpreted
	 *as longitudes.
	 * @exception GeographicErr if decoded latitude is not in
	 *[&minus;90&deg;, 90&deg;].
	 *
	 * By default, the \e lat (resp., \e lon) is assigned to the results of
	 * decoding \e dmsa (resp., \e dmsb).  However this is overridden if
	 *either \e dmsa or \e dmsb contain a latitude or longitude hemisphere
	 *designator (N, S, E, W).  If an exception is thrown, \e lat and \e lon
	 *are unchanged.
	 **********************************************************************/
	 ...
	 * @exception GeographicErr if \e angstr includes a hemisphere
	 *designator.
	 * @return angle (degrees)
	 *
	 * No hemisphere designator is allowed and no check is done on the range
	 *of the result.
	 **********************************************************************/
	 ...
	 * @param[in] trailing DMS::component value indicating the trailing
	 *units of the string (this component is given as a decimal number if
	 *   necessary).
	 ...
	 * @exception std::bad_alloc if memory for the string can't be
	 *allocated.
	 * @return formatted string
	 *
	 * The interpretation of \e ind is as follows:
	 * - ind == DMS::NONE, signed result no leading zeros on degrees except
	 *in the units place, e.g., -8d03'.
	 * - ind == DMS::LATITUDE, trailing N or S hemisphere designator, no
	 *sign, pad degrees to 2 digits, e.g., 08d03'S.
	 ...
	 * .
	 * The integer parts of the minutes and seconds components are always
	 *given with 2 digits.
	 **********************************************************************/
	static std::string Encode(real angle, component trailing, unsigned prec, flag ind = NONE, char dmssep = char(0));

	/**
	 * Convert angle into a DMS string (using d, ', and &quot;) selecting
	 *the trailing component based on the precision.
	 *
	 * @param[in] angle input angle (degrees)
	 ...
	 *3 gives a result accurate to 0.1' and \e prec = 4 gives a result
	 *accurate to 1&quot;.  \e ind is interpreted as in DMS::Encode with the
	 *additional facility that DMS::NUMBER represents \e angle as a number
	 *in fixed format with precision \e prec.
	 **********************************************************************/
	static std::string Encode(real angle, unsigned prec, flag ind = NONE, char dmssep = char(0))
	{
		return ind == NUMBER ? Utility::str(angle, int(prec))
		                     : Encode(angle, prec < 2 ? DEGREE : (prec < 4 ? MINUTE : SECOND),
		                              prec < 2 ? prec : (prec < 4 ? prec - 2 : prec - 4), ind, dmssep);
	}
	...
	{
		d = int(ang);
		ang = real(Math::dm) * (ang - d);
		m = int(ang);
		s = real(Math::ms) * (ang - m);
	}
};
