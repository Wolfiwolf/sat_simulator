...
private:
	...
	static real scale()
	{
		using std::pow;
		static const real
		    // Need extra real because, since C++11, pow(float, int)
		    // returns double
		    s = real(pow(
			real(std::numeric_limits<real>::radix),
			-3 * (std::numeric_limits<real>::max_exponent < (1 << 14) ? std::numeric_limits<real>::max_exponent : (1 << 14)) /
			    5));
		return s;
	}
	...
	{
		using std::sqrt;
		return std::numeric_limits<real>::epsilon() * sqrt(std::numeric_limits<real>::epsilon());
	}
...
public:
	 ...
	 *
	 * This packages up the \e C, \e S coefficients and information about
	 *how the coefficients are stored into a single structure.  This allows
	 *a vector of type SphericalEngine::coeff to be passed to
	...
	public:
		 ...
		 * @param[in] N the degree giving storage layout for \e C and \e
		 *S.
		 * @param[in] nmx the maximum degree to be used.
		 * @param[in] mmx the maximum order to be used.
		 * @exception GeographicErr if \e N, \e nmx, and \e mmx do not
		 *satisfy \e N &ge; \e nmx &ge; \e mmx &ge; &minus;1.
		 * @exception GeographicErr if \e C or \e S is not big enough to
		 *hold the coefficients.
		 * @exception std::bad_alloc if the memory for the square root
		 *table can't be allocated.
		 **********************************************************************/
		coeff(const std::vector<real> &C, const std::vector<real> &S, int N, int nmx, int mmx)
		    : _nNx(N), _nmx(nmx), _mmx(mmx), _cCnm(C.begin()), _sSnm(S.begin())
		{
			if (!((_nNx >= _nmx && _nmx >= _mmx && _mmx >= 0) ||
			      // If mmx = -1 then the sums are empty so require
			      // nmx = -1 also.
			      (_nmx == -1 && _mmx == -1)))
				throw GeographicErr("Bad indices for coeff");
			if (!(index(_nmx, _mmx) < int(C.size()) && index(_nmx, _mmx) < int(S.size()) + (_nNx + 1)))
		 ...
		 * @exception GeographicErr if \e N does not satisfy \e N &ge;
		 *&minus;1.
		 * @exception GeographicErr if \e C or \e S is not big enough to
		 *hold the coefficients.
		 * @exception std::bad_alloc if the memory for the square root
		...
		coeff(const std::vector<real> &C, const std::vector<real> &S, int N)
		    : _nNx(N), _nmx(N), _mmx(N), _cCnm(C.begin()), _sSnm(S.begin())
		{
			if (!(_nNx >= -1))
				throw GeographicErr("Bad indices for coeff");
			if (!(index(_nmx, _mmx) < int(C.size()) && index(_nmx, _mmx) < int(S.size()) + (_nNx + 1)))
		...
		/**
		 * @return \e N the degree giving storage layout for \e C and \e
		 *S.
		 **********************************************************************/
		int N() const
		{
		 ...
		 * @return the value of the \e C coefficient multiplied by \e f
		 *in \e n and \e m are in range else 0.
		 **********************************************************************/
		 ...
		 * @param[in] m the requested order.
		 * @param[in] f a multiplier.
		 * @return the value of the \e S coefficient multiplied by \e f
		 *in \e n and \e m are in range else 0.
		 **********************************************************************/
		 ...
		 * @return the size of the vector of cosine terms as stored in
		 *column major order.
		 **********************************************************************/
		 ...
		 * @param[in] truncate if false (the default) then \e N and \e M
		 *are determined by the values in the binary stream; otherwise,
		 *the input values of \e N and \e M are used to truncate the
		 *coefficients read from the stream at the given degree and
		 *order.
		 * @exception GeographicErr if \e N and \e M do not satisfy \e N
		 *&ge; \e M &ge; &minus;1.
		 * @exception GeographicErr if there's an error reading the
		 *data.
		 * @exception std::bad_alloc if the memory for \e C or \e S
		 *can't be allocated.
		 *
		 * \e N and \e M are read as 4-byte ints.  \e C and \e S are
		 *resized to accommodate all the coefficients (with the \e m = 0
		 *coefficients for \e S excluded) and the data for these
		 *coefficients read as 8-byte doubles.  The coefficients are
		 *stored in column major order.  The bytes in the stream should
		 *use little-endian ordering.  IEEE floating point is assumed
	 ...
	 * @tparam norm the normalization for the associated Legendre
	 *polynomials.
	 ...
	 * c[1].Cv + ... + f[L&minus;1] * c[L&minus;1].Cv.  (Note that f[0] is
	 *\e not used.)  The upper limits on the sum are determined by
	 *c[0].nmx() and c[0].mmx(); these limits apply to \e all the components
	 *of the coefficients.  The parameters \e gradp, \e norm, and \e L are
	 *template parameters, to allow more optimization to be done at compile
	 *time.
	 *
	 * Clenshaw summation is used which permits the evaluation of the sum
	 * without the need to allocate temporary arrays.  Thus this function
	 ...
	 * <i>y</i><sup>2</sup>), \e z, and \e a, it is more efficient to
	 *construct call SphericalEngine::Circle to give a CircularEngine object
	 *and then call CircularEngine::operator()() with arguments <i>x</i>/\e
	 *p and <i>y</i>/\e p.
	 **********************************************************************/
	template <bool gradp, normalization norm, int L>
	static CircularEngine Circle(const coeff c[], const real f[], real p, real z, real a);
	/**
	 * Check that the static table of square roots is big enough and enlarge
	 it
	 * if necessary.
	 *
	 ...
	 *   be allocated.
	 *
	 * Typically, there's no need for an end-user to call this routine,
	 because
	 * the constructors for SphericalEngine::coeff do so.  However, since
	 this
	 ...
	 * call this routine at program start up (when it's still single
	 threaded),
	 ...
	 * this only when you are sure you no longer will be using
	 *SphericalEngine. Your program will crash if you call SphericalEngine
	 *after calling this routine.
	 *
	 * \warning It's safest not to call this routine at all.  (The space
