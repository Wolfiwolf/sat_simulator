...
private:
	...
	static void Rotate(const real M[dim2_], real x, real y, real z, real &X, real &Y, real &Z)
	{
	...
	static void Unrotate(const real M[dim2_], real X, real Y, real Z, real &x, real &y, real &z)
	{
...
public:
	 ...
	 * @param[out] Y geocentric coordinate (meters).
	 * @param[out] Z geocentric coordinate (meters).
	 * @param[out] M if the length of the vector is 9, fill with the
	 ...
	 *to a local coordinate system at (\e lat, \e lon, \e h)); call this
	 *   representation \e v1.
	 * - in geocentric \e X, \e Y, \e Z coordinates; call this
	 *representation \e v0.
	 * .
	 ...
	 * solutions with different latitudes (applies only if \e Z = 0), then
	 *the solution with \e lat > 0 is returned.  If there are still multiple
	 * solutions with different longitudes (applies only if \e X = \e Y = 0)
	 * then \e lon = 0 is returned.  The value of \e h returned satisfies \e
	 *h &ge; &minus; \e a (1 &minus; <i>e</i><sup>2</sup>) / sqrt(1 &minus;
	...
	/**
	 * A global instantiation of Geocentric with the parameters for the
	 *WGS84 ellipsoid.
	 **********************************************************************/
