...
private:
	...
	CircularEngine(int M, bool gradp, unsigned norm, real a, real r, real u, real t)
	    : _mM(M), _gradp(gradp), _norm(norm), _a(a), _r(r), _u(u), _t(t), _wc(std::vector<real>(_mM + 1, 0)),
	      _ws(std::vector<real>(_mM + 1, 0)), _wrc(std::vector<real>(_gradp ? _mM + 1 : 0, 0)),
	      _wrs(std::vector<real>(_gradp ? _mM + 1 : 0, 0)), _wtc(std::vector<real>(_gradp ? _mM + 1 : 0, 0)),
...
public:
	 ...
	 * object returns zero.  The resulting object can be assigned to the
	 *result of SphericalHarmonic::Circle.
	 **********************************************************************/
	CircularEngine() : _mM(-1), _gradp(true), _u(0), _t(1)
	{
	}
