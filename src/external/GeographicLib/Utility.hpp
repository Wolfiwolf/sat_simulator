...
private:
	...
	{
		// The original cut over to the Gregorian calendar in Pope
		// Gregory XIII's time had 1582-10-04 followed by 1582-10-15.
		// Here we implement the switch over used by the
		// English-speaking world where 1752-09-02 was followed by
		// 1752-09-14. We also assume that the year always begins with
		// January 1, whereas in reality it often was reckoned to begin
		// in March.
...
public:
	 ...
	 * @param[in] m the month, Jan = 1, etc. (must be positive).  Default
	 *= 1.
	 ...
	 * return the numeric values for the year, month, and day.  No checking
	 *is done on these values.  The string "now" is interpreted as the
	 *present date (in UTC).
	 *
	 ...
	 *
	 * The string is first read as an ordinary number (e.g., 2010 or
	 *2012.5); if this is successful, the value is returned.  Otherwise the
	 *string should be of the form yyyy-mm or yyyy-mm-dd and this is
	 *converted to a number with 2010-01-01 giving 2010.0 and 2012-07-03
	 *giving 2012.5.  The string "now" is interpreted as the present date.
	 **********************************************************************/
	 ...
	 *
	 * If \e p &ge; 0, then the number fixed format is used with \e p bits
	 *of precision.  With \e p < 0, there is no manipulation of the format,
	 * except that <code>boolalpha</code> is used to represent bools as
	 *"true" and "false".  There is an overload of this function if T is
	 *Math::real; this deals with inf and nan.
	 **********************************************************************/
	 ...
	 *
	 * If T is bool, then \e s should either be string a representing 0
	 *(false) or 1 (true) or one of the strings
	 * - "false", "f", "nil", "no", "n", "off", or "" meaning false,
	 * - "true", "t", "yes", "y", or "on" meaning true;
	 * .
	 * case is ignored.
	 *
	 * If T is std::string, then \e s is returned (with the white space at
	 *the beginning and end removed).
	 **********************************************************************/
	template <typename T> static T val(const std::string &s)
	{
		// If T is bool, then the specialization val<bool>() defined
		// below is used.
		T x;
	 ...
	 *
	 * @tparam T the type of the return value (this should be a floating
	 *point type).
	 * @param[in] s the string to be matched.
	 * @return appropriate special value (&plusmn;&infin;, nan) or 0 if none
	 *is found.
	 *
	...
	/**
	 * Read a simple fraction, e.g., 3/4, from a string to an object of type
	 *T.
	 *
	 * @tparam T the type of the return value.
	 * @param[in] s the string to be converted.
	 * @exception GeographicErr is \e s is not readable as a fraction of
	 *type T.
	 * @return object of type T
	 *
	 ...
	 * "-1/300" gets mangled into something unrecognizable.  A workaround is
	 *to use a floating point number in the numerator, i.e., "-1.0/300".
	 *(Recent versions of the msys shell appear \e not to have this
	 *problem.)
	 **********************************************************************/
	 ...
	 **********************************************************************/
	template <typename ExtT, typename IntT, bool bigendp> static void readarray(std::istream &str, IntT array[], size_t num)
	{
#if GEOGRAPHICLIB_PRECISION < 4
		if (sizeof(IntT) == sizeof(ExtT) && std::numeric_limits<IntT>::is_integer == std::numeric_limits<ExtT>::is_integer) {
			// Data is compatible (aside from the issue of
			// endian-ness).
	 ...
	 * @exception GeographicErr if the data cannot be read.
	 **********************************************************************/
	template <typename ExtT, typename IntT, bool bigendp> static void readarray(std::istream &str, std::vector<IntT> &array)
	{
		if (array.size() > 0)
	 ...
	 * @tparam IntT the type of the objects in the array (internal).
	 * @tparam bigendp true if the external storage format is big-endian.
	 * @param[out] str the output stream for the data of type ExtT
	 *(external).
	 * @param[in] array the input array of type IntT (internal).
	 * @param[in] num the size of the array.
	 * @exception GeographicErr if the data cannot be written.
	 **********************************************************************/
	template <typename ExtT, typename IntT, bool bigendp> static void writearray(std::ostream &str, const IntT array[], size_t num)
	{
#if GEOGRAPHICLIB_PRECISION < 4
		if (sizeof(IntT) == sizeof(ExtT) && std::numeric_limits<IntT>::is_integer == std::numeric_limits<ExtT>::is_integer &&
	 ...
	 * @exception GeographicErr if the data cannot be written.
	 **********************************************************************/
	template <typename ExtT, typename IntT, bool bigendp> static void writearray(std::ostream &str, std::vector<IntT> &array)
	{
		if (array.size() > 0)
	 ...
	 *   non-NULL, this character and everything after it is discarded;
	 *default is '#'.
	 ...
	 * Use the \e equals delimiter character (or, if it is NULL -- the
	 *default, the first white space) to separate \e key and \e value.  \e
	 *key and \e value are trimmed of leading and trailing white space.  If
	 *\e key is empty, then \e value is set to "" and false is returned.
	 **********************************************************************/
	 ...
	 * cache the return value in a static local variable.  The precision
	 *needs to be set before a call to any such functions.)  In
	 *multi-threaded applications, it is necessary also to set the precision
	 *in each thread (see the example GeoidToGTX.cpp).
	 *
...
template <> inline std::string Utility::str<Math::real>(Math::real x, int p)
{
	...
	if (p == 0) {
		...
		if (2 * (ix - x) == 1 && fmod(ix, Math::real(2)) == 1)
			--ix;
