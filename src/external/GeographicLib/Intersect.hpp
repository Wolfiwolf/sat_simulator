...
public:
	/**
	 * The type used to hold the two displacement along the geodesics.  This
	 *is just a std::pair with \e x = \e first and \e y = \e second.
	 **********************************************************************/
	 ...
	 * this class.
	 **********************************************************************/
	static const unsigned LineCaps = Geodesic::LATITUDE | Geodesic::LONGITUDE | Geodesic::AZIMUTH | Geodesic::REDUCEDLENGTH |
	                                 Geodesic::GEODESICSCALE | Geodesic::DISTANCE_IN;
...
private:
	...
	public:
		real x, y;
		int c;
		XPoint(Math::real x, Math::real y, int c = 0) : x(x), y(y), c(c)
		{
		}
		XPoint() : x(Math::NaN()), y(Math::NaN()), c(0)
		{
		}
		XPoint(const Point &p) : x(p.first), y(p.second), c(0)
		{
		}
		XPoint &operator+=(const XPoint &p)
		{
			...
			y += p.y;
			if (p.c)
				c = p.c; // pass along a nonzero c from either
				         // operand
			return *this;
		}
		...
		Math::real Dist() const
		{
			return d1(x, y);
		}
		Math::real Dist(const XPoint &p) const
		{
	...
	public:
		...
		bool operator()(const XPoint &p, const XPoint &q) const
		{
			real dp = d1(p.x - _x, p.y - _y), dq = d1(q.x - _x, q.y - _y);
			return dp != dq ? (dp < dq) : (p.x != q.x ? (p.x < q.x) : (p.y < q.y));
		}
	};
	// The spherical solution
	XPoint Spherical(const GeodesicLine &lineX, const GeodesicLine &lineY, const XPoint &p) const;
	// The basic algorithm
	XPoint Basic(const GeodesicLine &lineX, const GeodesicLine &lineY, const XPoint &p0) const;
	// The closest intersecton
	XPoint ClosestInt(const GeodesicLine &lineX, const GeodesicLine &lineY, const XPoint &p0) const;
	// The next intersecton
	XPoint NextInt(const GeodesicLine &lineX, const GeodesicLine &lineY) const;
	// Segment intersecton
	XPoint SegmentInt(const GeodesicLine &lineX, const GeodesicLine &lineY, int &segmode) const;
	// All intersectons
	std::vector<XPoint> AllInt0(const GeodesicLine &lineX, const GeodesicLine &lineY, Math::real maxdist, const XPoint &p0) const;
	std::vector<Point> AllInternal(const GeodesicLine &lineX, const GeodesicLine &lineY, Math::real maxdist, const Point &p0,
	                               std::vector<int> &c, bool cp) const;
	// Find {semi-,}conjugate point which is close to s3.  Optional m12,
	// M12, M21 use {semi-,}conjugacy relative to point 2
	Math::real ConjugateDist(const GeodesicLine &line, Math::real s3, bool semi, Math::real m12 = 0, Math::real M12 = 1,
	                         Math::real M21 = 1) const;
	Math::real distpolar(Math::real lat1, Math::real *lat2 = nullptr) const;
	Math::real polarb(Math::real *lata = nullptr, Math::real *latb = nullptr) const;
	Math::real conjdist(Math::real azi, Math::real *ds = nullptr, Math::real *sp = nullptr, Math::real *sm = nullptr) const;
	Math::real distoblique(Math::real *azi = nullptr, Math::real *sp = nullptr, Math::real *sm = nullptr) const;
	...
	{
		return (p.x < 0 ? -1 : p.x <= sx ? 0 : 1) * 3 + (p.y < 0 ? -1 : p.y <= sy ? 0 : 1);
	}
...
public:
	 ...
	 *
	 * @param[in] geod a Geodesic object.  This sets the parameters \e a and
	 *\e f for the ellipsoid.
	 * @exception GeographicErr if the eccentricity of the elliposdoid is
	 *too large.
	 *
	 * \note This class has been validated for -1/4 &le; \e f &le; 1/5.  It
	 *may give satisfactory results slightly outside this range; however
	 * sufficient far outside the range, some internal checks will fail and
	 *an exception thrown.
	 *
	 ...
	 *
	 * @param[in] latX latitude of starting point for geodesic \e X
	 *(degrees).
	 * @param[in] lonX longitude of starting point for geodesic \e X
	 *(degrees).
	 * @param[in] aziX azimuth at starting point for geodesic \e X
	 *(degrees).
	 * @param[in] latY latitude of starting point for geodesic \e Y
	 *(degrees).
	 * @param[in] lonY longitude of starting point for geodesic \e Y
	 *(degrees).
	 * @param[in] aziY azimuth at starting point for geodesic \e Y
	 *(degrees).
	 ...
	 **********************************************************************/
	Point Closest(Math::real latX, Math::real lonX, Math::real aziX, Math::real latY, Math::real lonY, Math::real aziY,
	 ...
	 * The returned intersection minimizes Intersect::Dist(\e p, \e p0).
	 *
	 * \note \e lineX and \e lineY should be created with minimum
	 *capabilities Intersect::LineCaps.  The methods for creating a
	 *GeodesicLine include all these capabilities by default.
	 **********************************************************************/
	 ...
	 * @param[in] lonX2 longitude of second point for segment \e X
	 *(degrees).
	 ...
	 * @param[in] lonY2 longitude of second point for segment \e Y
	 *(degrees).
	 * @param[out] segmode an indicator equal to zero if the segments
	 *   intersect (see below).
	 * @param[out] c optional pointer to an integer coincidence indicator.
	 * @return \e p the intersection point if the segments intersect,
	 *otherwise the intersection point closest to the midpoints of the two
	 *   segments.
	 *
	 ...
	 **********************************************************************/
	Point Segment(Math::real latX1, Math::real lonX1, Math::real latX2, Math::real lonX2, Math::real latY1, Math::real lonY1,
	              Math::real latY2, Math::real lonY2, int &segmode, int *c = nullptr) const;
	/**
	 ...
	 *   segments.
	 *
	 * \warning \e lineX and \e lineY must represent shortest geodesics,
	 *e.g., they can be created by Geodesic::InverseLine.  The results are
	 *only well defined if there's a \e unique shortest geodesic between the
	 ...
	 *GeodesicLine include all these capabilities by default.
	 *
	 * See previous definition of Intersect::Segment for more information on
	 *\e segmode.
	 **********************************************************************/
	 ...
	 *
	 * @param[in] latX latitude of starting points for geodesics \e X and \e
	 *Y (degrees).
	 * @param[in] lonX longitude of starting points for geodesics \e X and
	 *\e Y (degrees).
	 * @param[in] aziX azimuth at starting point for geodesic \e X
	 *(degrees).
	 * @param[in] aziY azimuth at starting point for geodesic \e Y
	 ...
	 *
	 * The returned intersection minimizes Intersect::Dist(\e p) (excluding
	 *\e p = [0,0]).
	 *
	 * \note Equidistant closest intersections are surprisingly common.  If
	 * this may be a problem, use Intersect::All with a sufficiently large
	 *\e maxdist to capture close intersections.
	 **********************************************************************/
	 ...
	 * @return \e p the next closest intersection point.
	 *
	 * \warning \e lineX and \e lineY must both have the same starting
	 *point, i.e., the distance between [<i>lineX</i>.Latitude(),
	 ...
	 * @param[in] aziY azimuth at starting point for geodesic \e Y
	 *(degrees).
	 * @param[in] maxdist the maximum distance for the returned
	 ...
	 *
	 * Each intersection point satisfies Intersect::Dist(\e p, \e p0) &le;
	 *\e maxdist.  The vector of returned intersections is sorted on the
	 *distance from \e p0.
	 **********************************************************************/
	std::vector<Point> All(Math::real latX, Math::real lonX, Math::real aziX, Math::real latY, Math::real lonY, Math::real aziY,
	                       Math::real maxdist, std::vector<int> &c, const Point &p0 = Point(0, 0)) const;
	/**
	 * Find all intersections within a certain distance, with each geodesic
	 ...
	 **********************************************************************/
	std::vector<Point> All(Math::real latX, Math::real lonX, Math::real aziX, Math::real latY, Math::real lonY, Math::real aziY,
	                       Math::real maxdist, const Point &p0 = Point(0, 0)) const;
	/**
	 * Find all intersections within a certain distance, with each geodesic
	 ...
	 *GeodesicLine include all these capabilities by default.
	 **********************************************************************/
	std::vector<Point> All(const GeodesicLine &lineX, const GeodesicLine &lineY, Math::real maxdist, std::vector<int> &c,
	 ...
	 *GeodesicLine include all these capabilities by default.
	 **********************************************************************/
	std::vector<Point> All(const GeodesicLine &lineX, const GeodesicLine &lineY, Math::real maxdist,
	                       const Point &p0 = Point(0, 0)) const;
	///@}
	 ...
	 *
	 * This is a count of the number of times the spherical triangle needs
	 *to be solved.  Each involves a call to Geodesic::Inverse and this is a
	 *good metric for the overall cost. This counter is set to zero by the
	 * constructor.
	 *
	 ...
	 * which is used by all the intersection methods.  This counter is set
	 ...
	 *
	 * If this counter is incremented by just 1 in Intersect::Closest, then
	 *the initial result of the basic algorithm was eventually accepted.
	 ...
	 * intersection that overrides the intersection closest to the midpoints
	 *of the segments; i.e., NumCorner() always returns 0.
	 *
	 * \warning The counter is a mutable variable and so is not thread safe.
	 **********************************************************************/
