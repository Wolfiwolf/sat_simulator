...
private:
	...
	{
		_file.seekg(std::streamoff(_datastart + pixel_size_ * (unsigned(iy) * _swidth + unsigned(ix))));
	}
	real rawval(int ix, int iy) const
	{
		...
		if (_cache && iy >= _yoffset && iy < _yoffset + _ysize &&
		    ((ix >= _xoffset && ix < _xoffset + _xsize) || (ix + _width >= _xoffset && ix + _width < _xoffset + _xsize))) {
			return real(_data[iy - _yoffset][ix >= _xoffset ? ix - _xoffset : ix + _width - _xoffset]);
		} else {
			if (iy < 0 || iy >= _height) {
			...
			try {
				filepos(ix, iy);
				// initial values to suppress warnings in case
				// get fails
				char a = 0, b = 0;
				_file.get(a);
				_file.get(b);
				...
				// throw GeographicErr("Error reading " +
				// _filename + ": "
				//                      + e.what());
				// triggers complaints about the "binary '+'"
				// under Visual Studio. So use '+=' instead.
...
public:
	/**
	 * Flags indicating conversions between heights above the geoid and
	 *heights above the ellipsoid.
	 **********************************************************************/
	enum convertflag {
		/**
		 * The multiplier for converting from heights above the
		 *ellipsoid to heights above the geoid.
		 **********************************************************************/
		ELLIPSOIDTOGEOID = -1,
		/**
		 * No conversion.
		...
		NONE = 0,
		/**
		 * The multiplier for converting from heights above the geoid to
		 *heights above the ellipsoid.
		 **********************************************************************/
		GEOIDTOELLIPSOID = 1,
	};
	 ...
	 * @param[in] cubic (optional) interpolation method; false means
	 *bilinear, true (the default) means cubic.
	 ...
	 *is specified (and is non-empty), then the file is loaded from
	 *directory, \e path.  Otherwise the path is given by
	 *DefaultGeoidPath().  If the \e threadsafe parameter is true, the data
	 *set is read into memory, the data file is closed, and single-cell
	 *caching is turned off; this results in a Geoid object which \e is
	 *thread safe.
	 **********************************************************************/
	explicit Geoid(const std::string &name, const std::string &path = "", bool cubic = true, bool threadsafe = false);
	 ...
	 *   area.
	 * @param[in] west longitude (degrees) of the west edge of the cached
	 *area.
	 * @param[in] north latitude (degrees) of the north edge of the cached
	 *   area.
	 * @param[in] east longitude (degrees) of the east edge of the cached
	 *area.
	 * @exception GeographicErr if the memory necessary for caching the data
	 *   can't be allocated (in this case, you will have no cache and can
	 *try again with a smaller area).
	 ...
	 * parallels \e south and \e north and the meridians \e west and \e
	 *east. \e east is always interpreted as being east of \e west, if
	 *necessary by adding 360&deg; to its value.  \e south and \e north
	 ...
	 * @exception GeographicErr if this is called on a threadsafe Geoid.
	 *
	 * On most computers, this is fast for data sets with grid resolution of
	 *5' or coarser.  For a 1' grid, the required RAM is 450MB; a 2.5' grid
	 *needs 72MB; and a 5' grid needs 18MB.
	 **********************************************************************/
	void CacheAll() const
	{
		CacheArea(real(-Math::qd), real(0), real(Math::qd), real(Math::td));
	}

	/**
	 * Clear the cache.  This never throws an error.  (This does nothing
	 *with a thread safe Geoid.)
	 **********************************************************************/
	 ...
	 *   conversion; Geoid::GEOIDTOELLIPSOID means convert a height above
	 *the geoid to a height above the ellipsoid; Geoid::ELLIPSOIDTOGEOID
	 *means convert a height above the ellipsoid to a height above the
	 *geoid.
	 * @exception GeographicErr if there's a problem reading the data; this
	 *   never happens if (\e lat, \e lon) is within a successfully cached
	 *   area.
	...
	/**
	 * @return "name" used to load the geoid data (from the first argument
	 *of the constructor).
	 **********************************************************************/
	 ...
	 *   (meters).
	 *
	 * This relies on the value being stored in the data file.  If the value
	 *is absent, return &minus;1.
	 **********************************************************************/
	...
	{
		return _cache ? CacheWest() + (_xsize - (_xsize == _width ? 0 : 1 + 2 * _cubic)) / _rlonres : 0;
	}
