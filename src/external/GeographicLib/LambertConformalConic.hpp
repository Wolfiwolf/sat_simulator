...
private:
	...
	static real Dasinh(real x, real y, real hx, real hy)
	{
		...
		using std::asinh;
		real t = x - y;
		return t != 0 ? asinh(x * y > 0 ? t * (x + y) / (x * hy + y * hx) : x * hy - y * hx) / t : 1 / hx;
	}
...
public:
	 ...
	 *&minus; \e lat1) &le; 160&deg; and max(abs(\e lat1), abs(\e lat2))
	 *&le; 90 &minus; min(0.0002, 2.2 &times; 10<sup>&minus;6</sup>(180
	 *&minus; \e dlat), 6 &times 10<sup>&minus;8</sup>
	 *<i>dlat</i><sup>2</sup>) (in degrees), then the error in the latitude
	 *of origin is less than 4.5 &times; 10<sup>&minus;14</sup>d and the
	 *relative error in the scale is less than 7 &times;
	 *10<sup>&minus;15</sup>.
	 **********************************************************************/
	 ...
	 *LambertConformalConic::LatitudeOrigin(). No false easting or northing
	 *is added and \e lat should be in the range
	 ...
	 * meridian convergence and scale are consistent with this.  The values
	 *of \e x and \e y returned for points which project to infinity (i.e.,
	 *one or both of the poles) will be large but finite.
	 **********************************************************************/
	 ...
	 * The latitude origin is given by
	 *LambertConformalConic::LatitudeOrigin(). No false easting or northing
	 *is added.  The value of \e lon returned is in the range
	 *[&minus;180&deg;, 180&deg;].  The error in the projection is less than
	 *about 10 nm (10 nanometers), true distance, and the errors in the
	 *meridian convergence and scale are consistent with this.
	 **********************************************************************/
