...
#if defined(GEOGRAPHICLIB_HAVE_BOOST_SERIALIZATION) && GEOGRAPHICLIB_HAVE_BOOST_SERIALIZATION
 ...
 **********************************************************************/
template <typename dist_t, typename pos_t, class distfun_t> class NearestNeighbor
{
	// For tracking changes to the I/O format
	static const int version = 1;
	// This is what we get "free"; but if sizeof(dist_t) = 1 (unlikely),
	// allow 4 slots (and this accommodates the default value bucket = 4).
	static const int maxbucket = (2 + ((4 * sizeof(dist_t)) / sizeof(int) >= 2 ? (4 * sizeof(dist_t)) / sizeof(int) : 2));
...
public:
	 ...
	 * @param[in] bucket the size of the buckets at the leaf nodes; this
	 *must lie in [0, 2 + 4*sizeof(dist_t)/sizeof(int)] (default 4).
	 * @exception GeographicErr if the value of \e bucket is out of bounds
	 *or the size of \e pts is too big for an int.
	 ...
	 * larger \e bucket decreases the size of the NearestNeighbor object
	 *which scales as pts.size() / max(1, bucket) and reduces the number of
	 *distance calculations to construct the object by log2(bucket) *
	 *pts.size(). However each search then requires about bucket additional
	 *distance calculations.
	 *
	 * \warning The distances computed by \e dist must satisfy the standard
	 * metric conditions.  If not, the results are undefined.  Neither the
	 *data in \e pts nor the query points should contain NaNs or infinities
	 *because such data violates the metric conditions.
	 *
	...
	{
		static_assert(std::numeric_limits<dist_t>::is_signed, "dist_t must be a signed type");
		if (!(0 <= bucket && bucket <= maxbucket))
			throw GeographicLib::GeographicErr("bucket must lie in [0, 2 + "
			                                   "4*sizeof(dist_t)/sizeof(int)]");
		...
		init(pts, dist, bucket, tree, ids, cost, 0, int(ids.size()), int(ids.size() / 2));
		_tree.swap(tree);
	 ...
	 * @param[in] exhaustive whether to do an exhaustive search (default
	 *true).
	 * @param[in] tol the tolerance on the results (default 0).
	 * @return the distance to the closest point found (&minus;1 if no
	 *points are found).
	 * @exception GeographicErr if \e pts has a different size from that
	 *used to construct the object.
	 *
	 * The indices returned in \e ind are sorted by distance from \e query
	 * (closest first).
	 *
	 * The simplest invocation is with just the 4 non-optional arguments.
	 *This returns the closest distance and the index to the closest point
	 *in <i>ind</i><sub>0</sub>.  If there are several points equally close,
	 *then <i>ind</i><sub>0</sub> gives the index of an arbirary one of
	 *them.  If there's no closest point (because the set of points is
	 *empty), then \e ind is empty and &minus;1 is returned.
	 *
	 * With \e exhaustive = true and \e tol = 0 (their default values), this
	 * finds the indices of \e k closest neighbors to \e query whose
	 *distances to \e query are in (\e mindist, \e maxdist].  If \e mindist
	 *and \e maxdist have their default values, then these bounds have no
	 *effect.  If \e query is one of the points in the tree, then set \e
	 *mindist = 0 to prevent this point (and other coincident points) from
	 *being returned.
	 *
	 ...
	 * results are to be interpreted as follows: if the <i>k</i>'th distance
	 *is \e dk, then all results with distances less than or equal \e dk
	 *&minus; \e tol are correct; all others are suspect &mdash; there may
	 *be other closer results with distances greater or equal to \e dk
	 *&minus; \e tol. If less than \e k results are found, then the search
	 *is exact.
	 *
	 * \e mindist should be used to exclude a "small" neighborhood of the
	 *query point (relative to the average spacing of the data).  If \e
	 *mindist is large, the efficiency of the search deteriorates.
	 *
	 * \note Only the shortest distance is returned (as as the function
	 *value). The distances to other points (indexed by
	 *<i>ind</i><sub><i>j</i></sub> for \e j > 0) can be found by invoking
	 *\e dist again.
	 *
	 ...
	 * return some meaningless result (however, if the size of \e pts is
	 *wrong, this function throw an exception).
	 *
	 * \warning The query point cannot be a NaN or infinite because then the
	 * metric conditions are violated.
	 **********************************************************************/
	dist_t Search(const std::vector<pos_t> &pts, const distfun_t &dist, const pos_t &query, std::vector<int> &ind, int k = 1,
	              dist_t maxdist = std::numeric_limits<dist_t>::max(), dist_t mindist = -1, bool exhaustive = true,
	              dist_t tol = 0) const
	{
		...
		if (_numpoints > 0 && k > 0 && maxdist > mindist) {
			...
			dist_t tau = maxdist;
			// first is negative of how far query is outside
			// boundary of node +1 if on boundary or inside second
			// is node index
			...
			while (!todo.empty()) {
				...
				todo.pop();
				dist_t tau1 = tau - tol;
				// compare tau and d again since tau may have
				// become smaller.
				if (!(n >= 0 && tau1 >= d))
					continue;
				const Node &current = _tree[n];
				dist_t dst = 0; // to suppress warning about
				...
				tau1 = tau - tol;
				for (int l = 0; l < 2; ++l) {
					if (current.data.child[l] >= 0 && dst + current.data.upper[l] >= mindist) {
	 ...
	 * @exception std::bad_alloc if memory for the string representation of
	 *the object can't be allocated.
	 *
	 * The counters tracking the statistics of searches are not saved;
	 *however the initializtion cost is saved.  The format of the binary
	...
	{
		int realspec = std::numeric_limits<dist_t>::digits * (std::numeric_limits<dist_t>::is_integer ? -1 : 1);
		if (bin) {
			...
			for (int i = 0; i < int(_tree.size()); ++i) {
				...
				if (node.index >= 0) {
					os.write(reinterpret_cast<const char *>(node.data.lower), 2 * sizeof(dist_t));
					os.write(reinterpret_cast<const char *>(node.data.upper), 2 * sizeof(dist_t));
					os.write(reinterpret_cast<const char *>(node.data.child), 2 * sizeof(int));
				} else {
					os.write(reinterpret_cast<const char *>(node.leaves), _bucket * sizeof(int));
				}
			}
		} else {
			std::stringstream ostring;
			// Ensure enough precision for type dist_t.  With C++11,
			// max_digits10 can be used instead.
			if (!std::numeric_limits<dist_t>::is_integer) {
				static const int prec = int(std::ceil(std::numeric_limits<dist_t>::digits * std::log10(2.0) + 1));
				ostring.precision(prec);
			}
			ostring << version << " " << realspec << " " << _bucket << " " << _numpoints << " " << _tree.size() << " " << _cost;
			for (int i = 0; i < int(_tree.size()); ++i) {
				const Node &node = _tree[i];
				ostring << "\n" << node.index;
				if (node.index >= 0) {
	 ...
	 * operation.  Binary data must have been saved on a machine with the
	 *same architecture.  If an exception is thrown, the state of the
	...
	void Load(std::istream &is, bool bin = true)
	{
		...
		if (bin) {
			...
			if (!(is >> version1 >> realspec >> bucket >> numpoints >> treesize >> cost))
				throw GeographicLib::GeographicErr("Bad header");
		}
		if (!(version1 == version))
			throw GeographicLib::GeographicErr("Incompatible version");
		if (!(realspec == std::numeric_limits<dist_t>::digits * (std::numeric_limits<dist_t>::is_integer ? -1 : 1)))
		...
		for (int i = 0; i < treesize; ++i) {
			...
			if (bin) {
				...
				if (node.index >= 0) {
					is.read(reinterpret_cast<char *>(node.data.lower), 2 * sizeof(dist_t));
					is.read(reinterpret_cast<char *>(node.data.upper), 2 * sizeof(dist_t));
					is.read(reinterpret_cast<char *>(node.data.child), 2 * sizeof(int));
				} else {
					is.read(reinterpret_cast<char *>(node.leaves), bucket * sizeof(int));
				...
				if (node.index >= 0) {
					for (int l = 0; l < 2; ++l) {
						if (!(is >> node.data.lower[l] >> node.data.upper[l] >> node.data.child[l]))
							throw GeographicLib::GeographicErr("Bad node "
							                                   "data");
					}
				} else {
					// Must be at least one valid leaf
					// followed by a sequence end markers
					// (-1).
					for (int l = 0; l < bucket; ++l) {
						if (!(is >> node.leaves[l]))
							throw GeographicLib::GeographicErr("Bad leaf "
							                                   "data");
					}
					for (int l = bucket; l < maxbucket; ++l)
						node.leaves[l] = 0;
...
private:
	// Package up a dist_t and an int.  We will want to sort on the dist_t
	// so put it first.
	...
	public:
		struct bounds {
			dist_t lower[2],
			    upper[2]; // bounds on inner/outer distances
			int child[2];
		};
		union {
			bounds data;
		...
		void Check(int numpoints, int treesize, int bucket) const
		{
			if (!(-1 <= index && index < numpoints))
				throw GeographicLib::GeographicErr("Bad index");
			if (index >= 0) {
				if (!(-1 <= data.child[0] && data.child[0] < treesize && -1 <= data.child[1] && data.child[1] < treesize))
					throw GeographicLib::GeographicErr("Bad child pointers");
				if (!(0 <= data.lower[0] && data.lower[0] <= data.upper[0] && data.upper[0] <= data.lower[1] &&
				      data.lower[1] <= data.upper[1]))
					throw GeographicLib::GeographicErr("Bad bounds");
			} else {
				// Must be at least one valid leaf followed by a
				// sequence end markers
				// (-1).
				bool start = true;
				for (int l = 0; l < bucket; ++l) {
					if (!((start ? ((l == 0 ? 0 : -1) <= leaves[l] && leaves[l] < numpoints) : leaves[l] == -1)))
	...
	{
		int realspec = std::numeric_limits<dist_t>::digits * (std::numeric_limits<dist_t>::is_integer ? -1 : 1);
		// Need to use version1, otherwise load error in debug mode on
		// Linux: undefined reference to
		// GeographicLib::NearestNeighbor<...>::version.
		int version1 = version;
		ar &boost::serialization::make_nvp("version", version1) & boost::serialization::make_nvp("realspec", realspec) &
		    boost::serialization::make_nvp("bucket", _bucket) & boost::serialization::make_nvp("numpoints", _numpoints) &
		    boost::serialization::make_nvp("cost", _cost) & boost::serialization::make_nvp("tree", _tree);
	}
	template <class Archive> void load(Archive &ar, const unsigned)
	{
		...
		if (!(0 <= bucket && bucket <= maxbucket))
			throw GeographicLib::GeographicErr("Bad bucket size");
		ar &boost::serialization::make_nvp("numpoints", numpoints) & boost::serialization::make_nvp("cost", cost) &
	...
	int init(const std::vector<pos_t> &pts, const distfun_t &dist, int bucket, std::vector<Node> &tree, std::vector<item> &ids,
	         int &cost, int l, int u, int vp)
	{
		...
		if (u - l > (bucket == 0 ? 1 : bucket)) {
			...
			if (m > l + 1) { // node.child[0] is possibly empty
				typename std::vector<item>::iterator t = std::min_element(ids.begin() + l + 1, ids.begin() + m);
				node.data.lower[0] = t->first;
				t = std::max_element(ids.begin() + l + 1, ids.begin() + m);
				node.data.upper[0] = t->first;
				// Use point with max distance as vantage point;
				// this point act as a "corner" point and leads
				// to a good partition.
				node.data.child[0] = init(pts, dist, bucket, tree, ids, cost, l + 1, m, int(t - ids.begin()));
			}
			typename std::vector<item>::iterator t = std::max_element(ids.begin() + m, ids.begin() + u);
			node.data.lower[1] = ids[m].first;
			node.data.upper[1] = t->first;
			// Use point with max distance as vantage point here too
			node.data.child[1] = init(pts, dist, bucket, tree, ids, cost, m, u, int(t - ids.begin()));
		} else {
			if (bucket == 0)
				node.index = ids[l].second;
			else {
				node.index = -1;
				// Sort the bucket entries so that the tree is
