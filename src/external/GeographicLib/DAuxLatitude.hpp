...
public:
	 ...
	 *
	 * In the expression for the divided difference above, the angle
	 *quantities should be understood as the conventional measure of angle
	 *(either in radians or in degrees).
	 *
	 * The Fourier coefficients for a specific \e auxin and \e auxout are
	 * computed and saved on the first call; the saved coefficients are used
	 *on subsequent calls.  The series method is accurate for abs(\e f) &le;
	 * 1/150.
	 **********************************************************************/
	 ...
	 *   phi1), where \e psi = asinh( tan(\e chi) ) is the isometric
	 *latitude and \e chi is the conformal latitude.
	 *
	 * \note This routine uses the exact formulas and is valid for arbitrary
	 * latitude.
	 **********************************************************************/
	 ...
	 *
	 * \note This parameters for this routine are the \e tangents of
	 *conformal latitude.
	 *
	...
	static Math::real Dlam(real x, real y)
	{
		...
		return x == y ? base::sc(x)
		              : (isnan(x) || isnan(y)
		                     ...
		                     : (isinf(x) || isinf(y) ? std::numeric_limits<real>::infinity() : Dasinh(x, y) / Datan(x, y)));
	}
	// Dp0Dpsi in terms of chi
	/**
	 * The divided difference of the spherical rhumb area term with respect
	 *to the isometric latitude.
	 *
	 * @param[in] x tan(\e chi1).
	 * @param[in] y tan(\e chi2).
		...
		return x == y ? base::sn(x)
		              : (isnan(x + y) ? x + y : // N.B. nan for inf-inf
		                     (isinf(x) ? copysign(real(1), x)
		                               : (isinf(y) ? copysign(real(1), y) : Dasinh(h(x), h(y)) * Dh(x, y) / Dasinh(x, y))));
	}
