...
template <class GeodType = Geodesic> class PolygonAreaT
{
private:
	...
	template <typename T> void AreaReduce(T &area, int crossings, bool reverse, bool sign) const;

public:
	/**
	 * Constructor for PolygonAreaT.
	 *
	 * @param[in] earth the Geodesic object to use for geodesic
	 *calculations.
	 * @param[in] polyline if true that treat the points as defining a
	 *polyline instead of a polygon (default = false).
	 **********************************************************************/
	PolygonAreaT(const GeodType &earth, bool polyline = false)
	    : _earth(earth), _area0(_earth.EllipsoidArea()), _polyline(polyline),
	      _mask(GeodType::LATITUDE | GeodType::LONGITUDE | GeodType::DISTANCE |
	            (_polyline ? GeodType::NONE : GeodType::AREA | GeodType::LONG_UNROLL))
	{
		Clear();
	}
	 ...
	 * PolygonAreaT::CurrentPoint to determine the position of the new
	 *vertex.
	 **********************************************************************/
	 ...
	 *
	 * @param[in] reverse if true then clockwise (instead of
	 *counter-clockwise) traversal counts as a positive area.
	 * @param[in] sign if true then return a signed result for the area if
	 *   the polygon is traversed in the "wrong" direction instead of
	 *returning the area for the rest of the earth.
	 ...
	 *set if \e polyline is false in the constructor.
	 ...
	 * however, the data for the test point is not saved.  This lets you
	 *report a running result for the perimeter and area as the user moves
	 *the mouse cursor.  Ordinary floating point arithmetic is used to
	 *accumulate the data for the test point; thus the area and perimeter
	 *returned are less accurate than if PolygonAreaT::AddPoint and
	 ...
	 *   the polygon is traversed in the "wrong" direction instead of
	 *returning the area for the rest of the earth.
	 * @param[out] perimeter the approximate perimeter of the polygon or
	 *length of the polyline (meters).
	...
	/**
	 * Return the results assuming a tentative final test point is added via
	 *an azimuth and distance; however, the data for the test point is not
	 *saved. This lets you report a running result for the perimeter and
	 *area as the user moves the mouse cursor.  Ordinary floating point
	 *arithmetic is used to accumulate the data for the test point; thus the
	 *area and perimeter returned are less accurate than if
	 *PolygonAreaT::AddEdge and PolygonAreaT::Compute are used.
	 *
	 * @param[in] azi azimuth at current point (degrees).
	 * @param[in] s distance from current point to final test point
	 *(meters).
	 * @param[in] reverse if true then clockwise (instead of
	 *counter-clockwise) traversal counts as a positive area.
	 * @param[in] sign if true then return a signed result for the area if
