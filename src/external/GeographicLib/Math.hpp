...
public:
	 ...
	 * This is long double on computers with this type; otherwise it is
	 *double.
	 **********************************************************************/
	...
	/**
	 * The real type for %GeographicLib. Nearly all the testing has been
	 *done with \e real = double.  However, the algorithms should also work
	 *with float and long double (where available).  (<b>CAUTION</b>:
	 *reasonable accuracy typically cannot be obtained using floats.)
	 **********************************************************************/
	typedef double real;
	 ...
	 * &pi;/2.
	 *
	 * Defining all these in one place would mean that it's simple to
	 *convert to the centesimal system for measuring angles.  The DMS class
	 *assumes that Math::dm and Math::ms are less than or equal to 100 (so
	 *that two digits suffice for the integer parts of the minutes and
	 *degrees components of an angle).  Switching to the centesimal
	 *convention will break most of the tests.  Also the normal definition
	 *of degree is baked into some classes, e.g., UTMUPS, MGRS, Georef,
	 *Geohash, etc.
	 **********************************************************************/
	enum dms {
	...
	template <typename T> static void norm(T &x, T &y)
	{
		...
		// hypot for Visual Studio (A=win32) fails monotonicity, e.g.,
		// with
	 ...
	 *   <i>p</i><sub>0</sub> being coefficient of
	 *<i>x</i><sup><i>N</i></sup>.
	 ...
	 * <i>p</i><sub><i>n</i></sub>
	 *<i>x</i><sup><i>N</i>&minus;<i>n</i></sup>. Return 0 if \e N &lt; 0.
	 *Return <i>p</i><sub>0</sub>, if \e N = 0 (even if \e x is infinite or
	 *a nan).  The evaluation uses Horner's method.
	 **********************************************************************/
	template <typename T> static T polyval(int N, const T p[], T x)
	{
		// This used to employ Math::fma; but that's too slow and it
		// seemed not to improve the accuracy noticeably.  This might
		// change when there's direct hardware support for fma.
		T y = N < 0 ? 0 : *p++;
		while (--N >= 0)
			y = y * x + *p++;
		return y;
	 ...
	 *
	 * The range of \e x is unrestricted.  If the result is &plusmn;0&deg;
	 *or &plusmn;180&deg; then the sign is the sign of \e x.
	 **********************************************************************/
	 ...
	 * [&minus;180&deg;, 180&deg;]; and then sets \e z = \e d + \e e where
	 *\e d is the nearest representable number to \e z and \e e is the
	 *truncation error.  If \e z = &plusmn;0&deg; or &plusmn;180&deg;, then
	 *the sign of \e d is given by the sign of \e y &minus; \e x.  The
	 *maximum absolute value of \e e is 2<sup>&minus;26</sup> (for doubles).
	 **********************************************************************/
	 ...
	 *   180&deg;].
	 *
	 * The result is equivalent to computing the difference exactly,
	 *reducing it to [&minus;180&deg;, 180&deg;] and rounding the result.
	 **********************************************************************/
	 ...
	 *= 1/2<sup>57</sup> for doubles = 0.8 pm on the earth if \e x is an
	 *angle in degrees.  (This is about 2000 times more resolution than we
	 *get with angles around 90&deg;.)  We use this to avoid having to deal
	 *with near singular cases when \e x is non-zero but tiny (e.g.,
	 ...
	 *trigonometric functions, e.g., sin 9&deg; = cos 81&deg; = &minus; sin
	 *123456789&deg;. If x = &minus;0 or a negative multiple of 180&deg;,
	 *then \e sinx = &minus;0; this is the only case where &minus;0 is
	 *returned.
	 **********************************************************************/
	 ...
	 *
	 * This is a variant of Math::sincosd allowing a correction to the angle
	 *to be supplied.  \e x must be in [&minus;180&deg;, 180&deg;] and \e t
	 *is assumed to be a <i>small</i> correction.  Math::AngRound is applied
	 *to the reduced angle to prevent problems with \e x + \e t being
	 *extremely close but not exactly equal to one of the four cardinal
	 *directions.
	 **********************************************************************/
	 ...
	 * @return sin(<i>x</i>).
	 *
	 * The result is +0 for \e x = +0 and positive multiples of 180&deg;.
	 *The result is &minus;0 for \e x = -0 and negative multiples of
	 *180&deg;.
	 **********************************************************************/
